...
def test_all_new_connectors_importable():
    ...
    load_all_connectors()
    new_providers = [
        'workday', 'servicenow', 'knowbe4', 'snyk', 'purview', 'veeam',
        'intune', 'confluence', 'verkada', 'onetrust', 'proofpoint',
        'mlflow', 'vault', 'kubernetes', 'github', 'securityscorecard',
    ]
...
def test_all_new_normalizers_importable():
    ...
    expected = [
        'WorkdayNormalizer', 'ServiceNowNormalizer', 'KnowBe4Normalizer',
        'SnykNormalizer', 'PurviewNormalizer', 'VeeamNormalizer',
        'IntuneNormalizer', 'ConfluenceNormalizer', 'VerkadaNormalizer',
        'OneTrustNormalizer', 'ProofpointNormalizer', 'MLflowNormalizer',
        'VaultNormalizer', 'KubernetesNormalizer', 'GitHubNormalizer',
...
def test_workday_employees_normalizer():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"id": "W001", "status": "active", "descriptor": "Alice",
             "hireDate": "2024-01-15", "terminationDate": "",
             "department": "Engineering", "manager": "Bob"},
            {"id": "W002", "status": "active", "descriptor": "Charlie",
             "hireDate": "2023-06-01", "terminationDate": past_date,
             "department": "Sales", "manager": "Dana"},
        ]},
    )
...
def test_workday_background_checks():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"worker_id": "W001", "worker_name": "Alice",
             "background_check": {"status": "completed"}},
            {"worker_id": "W002", "worker_name": "Bob",
...
def test_servicenow_change_requests():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"sys_id": "s1", "number": "CHG001", "approval": "approved",
             "backout_plan": "Revert to v1.2", "short_description": "Upgrade DB",
             "type": "standard"},
            {"sys_id": "s2", "number": "CHG002", "approval": "not yet requested",
             "backout_plan": "", "short_description": "Deploy feature",
             "type": "normal"},
        ]},
    )
    findings = norm.normalize(raw)
...
def test_servicenow_incidents():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"sys_id": "i1", "number": "INC001", "state": "2", "priority": "1",
             "short_description": "Server down", "sla_due": past_sla},
        ]},
    )
    findings = norm.normalize(raw)
...
def test_knowbe4_training_enrollments():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"enrollment_id": "e1", "user": {"name": "Alice"},
             "module_name": "Security 101", "status": "Passed",
             "due_date": "", "completion_date": "2025-01-15"},
            {"enrollment_id": "e2", "user": {"name": "Bob"},
             "module_name": "Phishing 201", "status": "In Progress",
             "due_date": past_due, "completion_date": ""},
        ]},
    )
    findings = norm.normalize(raw)
    assert len(findings) >= 2
...
def test_knowbe4_training_campaigns():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"campaign_id": "c1", "name": "Q1 Training",
             "completion_percentage": 80, "status": "active"},
        ]},
    )
    findings = norm.normalize(raw)
...
def test_snyk_issues():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"id": "SNYK-JS-001", "title": "Prototype Pollution",
             "severity": "critical", "pkgName": "lodash",
             "version": "4.17.15", "fixedIn": ["4.17.21"]},
            {"id": "SNYK-JS-002", "title": "ReDoS", "severity": "low",
             "pkgName": "validator", "version": "10.0.0"},
        ]},
    )
    findings = norm.normalize(raw)
...
def test_purview_dlp_alerts():
    ...
    raw = RawEventData(
        ...
        raw_data={"records": [
            {"id": "a1", "title": "SSN detected in email", "severity": "high",
             "status": "new", "category": "DLP",
...
def test_purview_dlp_policies_disabled():
    ...
    raw = RawEventData(
        ...
        raw_data={"records": [
            {"id": "p1", "name": "Credit Card Policy", "isEnabled": False,
             "description": "Blocks credit card numbers"},
            {"id": "p2", "name": "SSN Policy", "isEnabled": True,
...
def test_veeam_backup_sessions():
    ...
    raw = RawEventData(
        ...
        raw_data={"records": [
            {"id": "s1", "name": "Daily Backup", "result": "success",
             "endTime": recent, "jobId": "j1"},
            {"id": "s2", "name": "Weekly Full", "result": "failed",
             "endTime": recent, "jobId": "j2"},
        ]},
    )
    findings = norm.normalize(raw)
    alerts = [f for f in findings if f.observation_type == "alert"]
...
def test_veeam_backup_jobs_disabled():
    ...
    raw = RawEventData(
        ...
        raw_data={"records": [
            {"id": "j1", "name": "Daily Backup", "type": "Backup",
             "isDisabled": True, "scheduleEnabled": False},
        ]},
    )
    findings = norm.normalize(raw)
    misconfigs = [f for f in findings if f.observation_type == "misconfiguration"]
...
def test_intune_devices():
    ...
    raw = RawEventData(
        ...
        raw_data={"records": [
            {"id": "D001", "deviceName": "LAPTOP-001",
             "complianceState": "compliant", "isEncrypted": True,
             "operatingSystem": "Windows", "osVersion": "11.0"},
            {"id": "D002", "deviceName": "LAPTOP-002",
...
def test_confluence_pages():
    ...
    raw = RawEventData(
        ...
        raw_data={"pages": [
            {"id": "p1", "title": "Security Policy", "status": "current",
             "authorId": "u1", "version": {"createdAt": stale_date}},
            {"id": "p2", "title": "Orphan Page", "status": "current",
             "authorId": "", "version": {"createdAt": stale_date}},
        ], "space_key": "SEC"},
    )
    findings = norm.normalize(raw)
    misconfigs = [f for f in findings if f.observation_type == "misconfiguration"]
...
def test_verkada_doors():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"door_id": "d1", "name": "Main Entrance",
             "lock_status": "locked", "site": "HQ"},
            {"door_id": "d2", "name": "Server Room",
             "lock_status": "unlocked", "site": "HQ"},
        ]},
    )
    findings = norm.normalize(raw)
    misconfigs = [f for f in findings if f.observation_type == "misconfiguration"]
...
def test_verkada_access_after_hours():
    ...
    norm = VerkadaNormalizer()
    # 3 AM is after hours
    after_hours = datetime.now(timezone.utc).replace(hour=3, minute=0).strftime("%Y-%m-%dT%H:%M:%SZ")
    raw = RawEventData(
        source="verkada", source_type=SourceType.PHYSICAL, provider="verkada",
        event_type="verkada_access_events",
        raw_data={"response": [
            {"event_id": "e1", "user_name": "Alice", "door_name": "Main",
             "event_time": after_hours, "event_type": "access_granted"},
        ]},
    )
    findings = norm.normalize(raw)
...
def test_onetrust_assessments():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"assessmentId": "a1", "name": "Widget PIA",
             "status": "in_progress", "type": "PIA"},
            {"assessmentId": "a2", "name": "Data Flow Review",
...
def test_onetrust_dsar_overdue():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"requestId": "r1", "subjectName": "John Doe",
             "status": "open", "type": "deletion",
...
def test_proofpoint_delivered_threats():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"GUID": "msg1", "subject": "Urgent invoice",
             "sender": "attacker@evil.com", "recipient": "user@co.com",
             "threatsInfoMap": {
...
def test_github_secret_scanning():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"number": 1, "secret_type": "aws_access_key_id", "state": "open",
...
def test_github_repos_public():
    ...
    raw = RawEventData(
        ...
        raw_data={"response": [
            {"id": 1, "full_name": "org/public-repo", "visibility": "public",
             "private": False, "default_branch": "main"},
            {"id": 2, "full_name": "org/private-repo", "visibility": "private",
...
def test_new_source_types():
    ...
    new_types = ['hris', 'itsm', 'training', 'physical', 'code',
                 'dlp', 'backup', 'mdm', 'grc', 'email']
    for t in new_types:
...
def test_api_modules_import():
    ...
    from warlock.api.auth import (
        hash_password, verify_password, generate_api_key, PERMISSIONS,
    )
...
def test_new_db_models():
    ...
    from warlock.db.models import (
        AuditEntry, PostureSnapshot, User, APIKey, AuditEngagement,
    )
...
def test_normalizers_reject_wrong_source():
    ...
    raw = RawEventData(
        source="unknown", source_type=SourceType.CUSTOM, provider="unknown",
        event_type="workday_employees", raw_data={},
    )
...
def test_normalizers_reject_wrong_event_type():
    ...
    raw = RawEventData(
        source="workday", source_type=SourceType.HRIS, provider="workday",
        event_type="nonexistent_event", raw_data={},
    )
    assert not WorkdayNormalizer().can_handle(raw)

...
def seeded(session):
    ...
    session.add(ConnectorRun(id="run-1", connector_name="aws", source="aws", source_type="cloud", provider="aws"))
    for i in range(5):
        session.add(RawEvent(id=f"raw-{i}", connector_run_id="run-1", source="aws", source_type="cloud",
                             provider="aws", event_type="iam_users", raw_data={}, sha256=f"r{i}"))
        session.add(Finding(id=f"f-{i}", raw_event_id=f"raw-{i}", observation_type="misconfiguration",
                            title=f"Finding {i}", detail={}, resource_type="iam_user",
                            source="aws", source_type="cloud", provider="aws",
                            severity="high", observed_at=now - timedelta(hours=i),
                            sha256=f"f{i}", system_profile_id="sys-1"))

    # Control mappings + results
    for i in range(5):
        session.add(ControlMapping(id=f"cm-{i}", finding_id=f"f-{i}", framework="nist_800_53",
                                    control_id="AC-2", control_family="AC",
                                    mapping_method="explicit", confidence=1.0, monitoring_frequency="daily"))
        status = "compliant" if i < 3 else "non_compliant"
        session.add(ControlResult(id=f"cr-{i}", finding_id=f"f-{i}", control_mapping_id=f"cm-{i}",
                                   framework="nist_800_53", control_id="AC-2",
                                   status=status, severity="high", assessor="assertion:test",
...
class TestPOAM:
    ...
    def test_list_and_overdue(self, seeded):
        ...
        seeded.add(POAM(
            id="overdue-1", framework="nist_800_53", control_id="AC-3",
            weakness_description="Test", severity="high", status="open",
...
class TestCompensatingControls:
    ...
    def test_list(self, seeded):
        ...
        mgr.create(seeded, original_framework="nist_800_53", original_control_id="AC-2",
                    title="CC1", description="Desc", status="active")
        mgr.create(seeded, original_framework="soc2", original_control_id="CC6.1",
                    title="CC2", description="Desc", status="proposed")
        seeded.flush()
...
class TestRiskAcceptance:
    ...
    def test_expired(self, seeded):
        ...
        seeded.add(RiskAcceptance(
            id="expired-1", framework="nist_800_53", control_id="AC-3",
            risk_description="Test", risk_level="low",
            requested_by="user@test.com", status="active",
...
class TestFrameworkDiff:
    ...
    def test_diff_changes(self, tmp_path):
        ...
        new_config = {
            ...
            "control_families": {
                "AC": {"controls": {
                    ...
                    "AC-2": {"checks": [{"id": "new_check", "event_types": ["iam_users", "okta_users"]}]},  # modified
        ...
        modified_ids = [
            c.control_id if hasattr(c, 'control_id') else c
            for c in result.modified_controls
        ]
...
class TestPhase35Schema:
    ...
    def test_new_tables_exist(self, session):
        ...
        expected = {
            "poams", "compensating_controls", "risk_acceptances",
            "control_inheritances", "system_dependencies",
            "change_events", "compliance_drifts",
            "policy_overrides", "external_auditors",
            "auditor_engagement_assignments", "evidence_requests",
        }
        for t in expected:

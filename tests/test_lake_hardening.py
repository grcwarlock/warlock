...
def lake_with_posture(tmp_path):
    ...
    for fw in ["nist_800_53", "soc2"]:
        ...
        fw_data = {
            k: [v for v, f in zip(snapshots[k], snapshots["framework"]) if f == fw]
            for k in snapshots
        }
...
class TestABACLakeReaders:
    ...
    def test_dashboard_system_profile_scope(self, lake_for_abac):
        ...
        readers = LakeReaders(lake_for_abac)
        result = readers.dashboard_framework_summary(allowed_system_profiles=["sys-prod"])
    ...
    def test_distinct_frameworks_scoped(self, lake_for_abac):
        ...
        readers = LakeReaders(lake_for_abac)
        result = readers.distinct_frameworks(allowed_frameworks=["soc2", "iso_27001"])
    ...
    def test_list_controls_scoped(self, lake_for_abac):
        ...
        result = readers.list_controls("nist_800_53", allowed_frameworks=["soc2"])
        assert len(result) == 0
        readers.close()
    ...
    def test_top_non_compliant_risks_no_match(self, lake_for_abac):
        ...
        result = readers.top_non_compliant_risks(allowed_frameworks=["nist_800_53"])
        assert len(result) == 0
        readers.close()
    ...
    def test_profile_scope_excludes_staging(self, lake_for_abac):
        ...
        result = readers.coverage_by_status(allowed_system_profiles=["sys-prod"])
        frameworks = {r[0] for r in result}
        assert "iso_27001" not in frameworks
...
class TestBridgeTables:
    def test_write_crosswalk(self, tmp_path):
        ...
        result = engine.query(
            f"SELECT * FROM read_parquet('{tmp_path}/curated/bridge_control_crosswalk/**/*.parquet')"
        )
        assert len(result) == 1
    ...
    def test_write_entity_relationships(self, tmp_path):
        ...
        relationships = [
            {
                "source_entity": "user-1",
                "target_entity": "laptop-1",
    ...
    def test_write_incident_bridges(self, tmp_path):
        ...
        count = write_bridge_tables(
            str(tmp_path), "run-1", incident_controls=controls, incident_entities=entities
        )
        assert count == 2
    ...
    def test_write_data_flow(self, tmp_path):
        ...
        flows = [
            {
                "source_entity": "app-1",
                "destination_entity": "db-1",
                "data_classification": "PII",
                "transfer_mechanism": "TLS",
                "legal_basis": "consent",
...
class TestTypedParquetColumns:
    ...
    def test_integer_fields_stored_as_int(self, tmp_path):
        ...
        runs = [
            {
                "id": "pr-1",
                "run_id": "run-1",
                "raw_events_collected": 100,
                "findings_normalized": 50,
                "controls_mapped": 200,
                "results_assessed": 200,
        ...
        assert result[0]["t"] in ("INTEGER", "BIGINT"), (
            f"raw_events_collected stored as {result[0]['t']}"
        )
        engine.close()
...
class TestSCDType2:
    ...
    def test_changed_entity_closes_old(self):
        ...
        result = apply_scd_type2(existing, incoming, key_fields=["id"], change_date="2026-03-21")
        assert len(result) == 2
...
class TestLakeRAG:
    ...
    def seeded_lake_for_rag(self, tmp_path):
        ...
        f = {
            "id": ["f-1", "f-2"],
            "title": ["No MFA enabled", "Disk unencrypted"],
            "severity": ["high", "critical"],
            "source": ["aws", "aws"],
            "observation_type": ["iam_user", "ebs_volume"],
            "raw_event_id": ["r-1", "r-2"],
            "detail": ["", ""],
            "resource_id": ["u-1", "v-1"],
            "resource_type": ["user", "volume"],

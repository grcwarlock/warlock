...
class TestTemporalFacts:
    def test_posture_snapshots(self, lake_dir):
        rows = [
            {"id": "1", "framework": "nist_800_53", "control_id": "AC-1",
             "system_profile_id": "sp-1", "snapshot_date": "2026-03-21",
             "posture_score": "0.85", "status": "compliant"},
        ]
    ...
    def test_compliance_drifts(self, lake_dir):
        rows = [
            {"id": "1", "framework": "soc2", "control_id": "CC6.1",
             "previous_status": "compliant", "new_status": "non_compliant",
             "drift_direction": "regression", "detected_at": "2026-03-21T00:00:00Z"},
        ]
    ...
    def test_regulatory_deadlines(self, lake_dir):
        rows = [
            {"id": "1", "regulation": "GDPR", "deadline_type": "dsar_response",
             "deadline_at": "2026-04-20", "days_remaining": "30",
             "status": "pending", "linked_control_ids": "PR-1,PR-2"},
        ]
...
class TestRiskFacts:
    def test_risk_simulations(self, lake_dir):
        rows = [
            {"id": "1", "framework": "nist_800_53", "scenario_name": "ransomware",
             "mean_ale": "500000", "var_95": "1200000", "var_99": "2000000",
             "control_effectiveness": "0.72", "created_at": "2026-03-21"},
        ]
    ...
    def test_vulnerability_lifecycle(self, lake_dir):
        rows = [
            {"id": "1", "vuln_id": "CVE-2026-1234", "cvss_score": "9.8",
             "epss_score": "0.95", "status": "remediated",
             "remediated_at": "2026-03-15", "sla_target_days": "7", "sla_met": "True"},
        ]
    ...
    def test_control_effectiveness(self, lake_dir):
        rows = [
            {"id": "1", "control_id": "AC-2", "framework": "nist_800_53",
             "effectiveness_rate": "0.91", "trend": "improving",
...
class TestEntityFacts:
    def test_resources(self, lake_dir):
        rows = [
            {"id": "1", "resource_type": "ec2", "resource_id": "i-abc123",
             "account_id": "111222333", "region": "us-east-1",
             "tags": "{}", "valid_from": "2026-01-01", "valid_to": "",
             "is_current": "True"},
        ]
    ...
    def test_personnel(self, lake_dir):
        rows = [
            {"id": "1", "email": "j@example.com", "department": "eng",
             "hr_status": "active", "mfa_enabled": "True",
             "risk_score": "0.1", "valid_from": "2026-01-01",
             "valid_to": "", "is_current": "True"},
        ]
...
class TestGovernanceFacts:
    def test_poams(self, lake_dir):
        rows = [
            {"id": "1", "framework": "nist_800_53", "control_id": "AC-2",
             "status": "open", "milestone": "Deploy MFA",
             "owner": "security-team", "created_at": "2026-03-01"},
        ]
    ...
    def test_audit_entries(self, lake_dir):
        rows = [
            {"id": "1", "sequence": "100", "action": "create",
             "entity_type": "ControlResult", "entity_id": "cr-1",
             "actor": "system", "entry_hash": "abc123",
...
class TestEvidenceFacts:
    def test_evidence_artifacts(self, lake_dir):
        rows = [
            {"id": "1", "source_connector": "aws_config",
             "collected_at": "2026-03-21", "artifact_type": "screenshot",
             "storage_ref": "s3://bucket/key", "hash": "sha256:abc",
    ...
    def test_evidence_control_bindings(self, lake_dir):
        rows = [
            {"evidence_id": "1", "control_id": "AC-1",
             "framework": "nist_800_53", "sufficiency_score": "0.9"},
        ]
...
class TestPrivacyFacts:
    def test_processing_activities(self, lake_dir):
        rows = [
            {"id": "1", "purpose": "marketing", "legal_basis": "consent",
             "data_categories": "email,name", "recipients": "CRM vendor",
    ...
    def test_dsars(self, lake_dir):
        rows = [
            {"id": "1", "request_type": "access", "received_at": "2026-03-01",
             "deadline_at": "2026-03-31", "completed_at": "", "status": "in_progress"},
        ]
    ...
    def test_breach_register(self, lake_dir):
        rows = [
            {"id": "1", "discovered_at": "2026-03-20",
             "reported_at": "2026-03-21", "affected_subjects_count": "1500",
...
class TestIncidentFacts:
    def test_security_events(self, lake_dir):
        rows = [
            {"id": "1", "source_connector": "crowdstrike",
             "event_type": "malware", "severity": "critical",
    ...
    def test_incidents(self, lake_dir):
        rows = [
            {"id": "1", "classification": "data_breach", "severity": "high",
             "status": "investigating", "commander": "ciso",
    ...
    def test_tabletop_exercises(self, lake_dir):
        rows = [
            {"id": "1", "scenario": "ransomware", "participants": "exec-team",
             "findings": "slow response", "action_items": "update runbook"},
        ]
...
class TestPipelineHealthFacts:
    def test_pipeline_runs(self, lake_dir):
        rows = [
            {"id": "1", "run_id": "run-42", "raw_events_collected": "222",
             "findings_normalized": "646", "controls_mapped": "32817",
             "results_assessed": "32817", "duration_seconds": "12.5",
    ...
    def test_data_freshness(self, lake_dir):
        rows = [
            {"id": "1", "connector_id": "aws_config",
             "last_successful_run": "2026-03-21T06:00:00Z",
             "hours_since_last_success": "2", "freshness_status": "fresh"},
        ]
    ...
    def test_coverage_metrics(self, lake_dir):
        rows = [
            {"id": "1", "total_connectors": "57", "healthy": "55",
             "degraded": "2", "failed": "0",
...
class TestSupplyChainFacts:
    def test_sbom_components(self, lake_dir):
        rows = [
            {"id": "1", "component_name": "openssl", "version": "3.0.13",
             "sbom_format": "CycloneDX", "license": "Apache-2.0",
             "known_cves": "0", "slsa_level": "3"},
        ]
    ...
    def test_supplier_assessments(self, lake_dir):
        rows = [
            {"id": "1", "supplier_id": "vendor-1",
             "assessed_at": "2026-03-01", "rating_source": "bitsight",
             "score": "740"},
        ]
    ...
    def test_concentration_risk(self, lake_dir):
        rows = [
            {"id": "1", "supplier_id": "aws", "dependent_systems": "12",
    ...
    def test_provenance_attestations(self, lake_dir):
        rows = [
            {"id": "1", "artifact_id": "pkg-1", "slsa_level": "2",
             "signer": "ci-bot", "signature_verified": "True"},
        ]
...
class TestCrossDomain:
    def test_multiple_rows(self, lake_dir):
        ...
        rows = [{"id": str(i), "framework": "soc2", "control_id": f"CC{i}.1",
                 "status": "open", "milestone": "", "owner": "",
                 "created_at": "2026-03-21"} for i in range(1, 11)]
        count = write_governance_facts(lake_dir, RUN_ID, poams=rows)
        assert count == 10
    ...
    def test_framework_partitioning(self, lake_dir):
        ...
        rows = [
            {"id": "1", "framework": "nist_800_53", "control_id": "AC-1",
             "system_profile_id": "sp-1", "snapshot_date": "2026-03-21",
             "posture_score": "0.85", "status": "compliant"},
            {"id": "2", "framework": "soc2", "control_id": "CC6.1",
             "system_profile_id": "sp-1", "snapshot_date": "2026-03-21",
             "posture_score": "0.9", "status": "compliant"},
        ]
        count = write_temporal_facts(lake_dir, RUN_ID, posture_snapshots=rows)
        assert count == 2

"""Incidents view — issue tracking with severity/status/assignee columns."""

from __future__ import annotations

from typing import Any

from rich.markup import escape
from textual.app import ComposeResult
from textual.binding import Binding
from textual.containers import Horizontal, Vertical, VerticalScroll
from textual.reactive import reactive
from textual.widget import Widget
from textual.widgets import ListItem, ListView, Static

from warlock.utils import ensure_aware

# ------------------------------------------------------------------ #
# Helpers                                                              #
# ------------------------------------------------------------------ #

PRIORITY_STYLE = {
    "critical": "bold red",
    "high": "dark_orange",
    "medium": "yellow",
    "low": "dim",
}

STATUS_STYLE = {
    "open": "yellow",
    "assigned": "cyan",
    "in_progress": "dodger_blue2",
    "remediated": "medium_purple",
    "verified": "green",
    "closed": "green",
    "risk_accepted": "dark_orange",
}


def _priority(s: str) -> str:
    style = PRIORITY_STYLE.get(s, "white")
    return f"[{style}]{s:<8}[/]"


def _status(s: str) -> str:
    style = STATUS_STYLE.get(s, "white")
    return f"[{style}]{s:<14}[/]"


def _ts(dt) -> str:
    if dt is None:
        return "[dim]\u2014[/]"
    aware = ensure_aware(dt)
    return f"[dim]{aware:%Y-%m-%d}[/]"


# ------------------------------------------------------------------ #
# List row                                                             #
# ------------------------------------------------------------------ #


class IncidentRow(ListItem):
    """A single incident in the list."""

    def __init__(self, data: dict[str, Any]) -> None:
        self.data = data
        super().__init__()

    def compose(self) -> ComposeResult:
        d = self.data
        title = escape(d["title"])
        if len(title) > 44:
            title = title[:41] + "..."
        assignee = escape(d["assigned_to"][:12]) if d["assigned_to"] else "[dim]\u2014[/]"
        line = (
            f"{_priority(d['priority'])} {_status(d['status'])} "
            f"{title:<44}  {assignee:<12}  {_ts(d.get('created_at'))}"
        )
        yield Static(line)


# ------------------------------------------------------------------ #
# Detail pane                                                          #
# ------------------------------------------------------------------ #


class IncidentDetailPane(Widget):
    """Right-side detail pane for a selected incident."""

    item: reactive[dict | None] = reactive(None, layout=True)

    def render(self) -> str:
        if self.item is None:
            return "[dim]Select an incident[/]"
        d = self.item
        lines: list[str] = []

        lines.append(f"[bold #a78bfa]{escape(d['title'])}[/]")
        lines.append("")
        lines.append(f"  Priority     {_priority(d['priority'])}")
        lines.append(f"  Status       {_status(d['status'])}")
        assignee = escape(d["assigned_to"]) if d["assigned_to"] else "[dim]unassigned[/]"
        lines.append(f"  Assigned To  {assignee}")
        lines.append(f"  Created      {_ts(d.get('created_at'))}")

        if d.get("framework"):
            lines.append(
                f"  Framework    [#a78bfa]{escape(d['framework'])} "
                f"{escape(d.get('control_id', ''))}[/]"
            )
        if d.get("source"):
            lines.append(f"  Source       [dim]{escape(d['source'])}[/]")
        lines.append("")

        if d.get("description"):
            lines.append("[bold #a78bfa]\u25c6 DESCRIPTION[/]")
            desc = escape(d["description"])
            if len(desc) > 300:
                desc = desc[:297] + "..."
            lines.append(f"  {desc}")
            lines.append("")

        if d.get("remediation_plan"):
            lines.append("[bold #a78bfa]\u25c6 REMEDIATION PLAN[/]")
            lines.append(f"  {escape(d['remediation_plan'])}")
            lines.append("")

        if d.get("risk_accepted"):
            lines.append("[dark_orange]\u26a0 Risk Accepted[/]")
            lines.append("")

        overdue = d.get("overdue", False)
        if overdue:
            lines.append("[bold red]\u26a0 OVERDUE[/]")
            lines.append("")

        fid = d.get("id", "")
        if fid:
            lines.append(f"  [dim]ID: {fid}[/]")

        return "\n".join(lines)


# ------------------------------------------------------------------ #
# View                                                                 #
# ------------------------------------------------------------------ #


class IncidentsView(Vertical):
    """Incidents list with detail pane."""

    can_focus = True

    BINDINGS = [
        Binding("j", "cursor_down", "Down", show=False),
        Binding("k", "cursor_up", "Up", show=False),
        Binding("enter", "select_item", "Select", show=False),
        Binding("escape", "go_back", "Back", show=False),
        Binding("r", "refresh_data", "Refresh", show=False),
    ]

    _items: reactive[list[dict]] = reactive(list, layout=True)

    def compose(self) -> ComposeResult:
        yield Static("", id="header-bar")
        with Horizontal():
            with Vertical(id="list-panel"):
                yield ListView(id="incident-list")
            yield VerticalScroll(IncidentDetailPane(id="detail-view"), id="detail-pane")
        yield Static("", id="footer-bar")

    def on_mount(self) -> None:
        self.focus()
        self._load_data()

    def _load_data(self) -> None:
        self.run_worker(self._fetch_data, thread=True)

    def _fetch_data(self) -> None:
        try:
            from warlock.tui.data.queries import get_incident_counts, get_incidents

            items = get_incidents()
            counts = get_incident_counts()
            self.app.call_from_thread(self._set_data, items, counts)
        except Exception as e:
            try:
                self.app.call_from_thread(self._set_error, str(e))
            except Exception:
                pass

    def _set_data(self, items: list[dict], counts: dict) -> None:
        self._items = items

        header = self.query_one("#header-bar", Static)
        header.update(
            f" [bold]Incidents[/]  [dim]{counts['total']} total[/]"
            f"    [on dark_red] {counts['open']} open [/]"
            f"  [on dark_red] {counts['critical']} critical [/]"
        )

        footer = self.query_one("#footer-bar", Static)
        footer.update(
            " [#a78bfa]j[/]/[#a78bfa]k[/] move  "
            "[#a78bfa]Enter[/] select  "
            "[#a78bfa]r[/] refresh  "
            "[#a78bfa]Esc[/] back"
        )

        lv = self.query_one("#incident-list", ListView)
        lv.clear()
        for item in items:
            lv.append(IncidentRow(item))

        if items:
            lv.index = 0
            lv.focus()
            self._update_detail(items[0])

    def _set_error(self, error: str) -> None:
        header = self.query_one("#header-bar", Static)
        header.update(f" [bold red]Error loading incidents:[/] {escape(error)}")

    def on_list_view_selected(self, event: ListView.Selected) -> None:
        if isinstance(event.item, IncidentRow):
            self._update_detail(event.item.data)

    def on_list_view_highlighted(self, event: ListView.Highlighted) -> None:
        if event.item and isinstance(event.item, IncidentRow):
            self._update_detail(event.item.data)

    def _update_detail(self, item: dict) -> None:
        detail = self.query_one("#detail-view", IncidentDetailPane)
        detail.item = item

    def action_select_item(self) -> None:
        lv = self.query_one("#incident-list", ListView)
        if lv.highlighted_child and isinstance(lv.highlighted_child, IncidentRow):
            self._update_detail(lv.highlighted_child.data)

    def action_go_back(self) -> None:
        pass

    def action_cursor_down(self) -> None:
        self.query_one("#incident-list", ListView).action_cursor_down()

    def action_cursor_up(self) -> None:
        self.query_one("#incident-list", ListView).action_cursor_up()

    def action_refresh_data(self) -> None:
        self._load_data()
        self.notify("Refreshed")

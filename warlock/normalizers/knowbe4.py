...
class KnowBe4Normalizer(BaseNormalizer):
    ...
    def _normalize_training_enrollments(self, raw: RawEventData) -> list[FindingData]:
        ...
        for enrollment in enrollments:
            ...
            user_name = enrollment.get("user", {}).get("name", "") or enrollment.get(
                "user_name", "unknown"
            )
    ...
    def _normalize_phishing_results(self, raw: RawEventData) -> list[FindingData]:
        ...
        for test in tests:
            ...
            for recipient in recipients:
                user_name = recipient.get("user", {}).get("name", "") or recipient.get(
                    "email", "unknown"
                )
        ...
        for user in users:
            ...
            findings.append(
                FindingData(
                    **self._base(raw),
                    observation_type=obs_type,
                    title=f"KnowBe4 user: {name}"
                    + (f" -- risk score {risk_score}" if issues else ""),
                    detail={
                        "user_id": user_id,
                        "name": name,
                        "email": email,

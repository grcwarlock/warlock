...
for enh, summ, steps, cp in [
    ...
    ("2", "Processing domains for information flow", [
    ...
    ("9", "Human reviews for flow control", [
    ...
    ("27", "Redundant or independent filtering mechanisms", [
    ...
    ("31", "Failed content transfer prevention", [
    ...
    ("32", "Process requirements for information transfer", [
...
for enh, summ, steps, cp in [
    ("1", "Automatic account lock from failed attempts", [
    ...
    ("2", "Purge or wipe device after failed logon attempts", [
    ...
    ("4", "Use of alternate factor for account unlock", [
...
for enh, summ, steps, cp in [
    ("1", "Monitoring and control of remote access", [
    ...
    ("9", "Disconnect or disable remote access", [
    ...
    ("10", "Authenticate remote commands", [
...
for enh, summ, steps in [
    ("1", "Authentication and encryption for wireless", [
    ...
    ("2", "Monitoring for unauthorized wireless connections", [
    ...
    ("5", "Antennas and transmission power levels", [
...
CONTROLS["AC-19"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: AC-19 Assessment Procedures", "NIST SP 800-124: Mobile Device Security"],
...
for enh, summ, steps in [
    ...
    ("5", "Full device or container-based encryption", [
...
for enh, summ, steps in [
    ...
    ("2", "Portable storage devices on external systems", [
    ...
    ("3", "Non-organizationally owned systems as components", [
    ...
    ("4", "Network accessible storage devices as components", [
...
CONTROLS["AT-1"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: AT-1 Assessment Procedures", "NIST SP 800-50: Building an IT Security Awareness and Training Program"],
    "evidence_types": ["policy_document", "policy_review_date"],
}
...
CONTROLS["AT-2"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: AT-2 Assessment Procedures", "NIST SP 800-50: Security Awareness Training"],
    "evidence_types": ["training_records", "phishing_results"],
}

for enh, summ, steps in [
    ("1", "Practical exercises in awareness training", [
    ...
    ("3", "Social engineering and mining awareness", [
    ...
    ("4", "Suspicious communications and anomalous behavior", [
...
for enh, summ, steps in [
    ("1", "Environmental controls training", [
    ...
    ("2", "Physical security controls training", [
    ...
    ("3", "Practical exercises in role-based training", [
    ...
    ("5", "Accessing personally identifiable information", [
...
for enh, summ, steps in [
    ("1", "Training feedback — awareness training", [
    ...
    ("2", "Training feedback — role-based training", [
...
CONTROLS["AU-2"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: AU-2 Assessment Procedures", "NIST SP 800-92: Guide to Computer Security Log Management"],
    "evidence_types": ["cloudtrail_trails", "audit_logs", "activity_log"],
}

for enh, summ, steps in [
    ("1", "Compilation of audit records from multiple sources", [
    ...
    ("2", "Selection of audit events by component", [
    ...
    ("3", "Reviews and updates to audited events", [
...
for enh, summ, steps in [
    ...
    ("3", "Limit personally identifiable information in audit records", [
...
for enh in range(1, 11):
    ...
    steps_map = {
        1: ["Implement automated audit analysis in SIEM", "Configure automated alert generation", "Set up scheduled audit review reports", "Verify automated analysis coverage"],
        2: ["Configure real-time security alerts in SIEM", "Set up alert notification channels", "Define alert severity levels and response procedures", "Test alert delivery monthly"],
        3: ["Correlate audit records across multiple log sources", "Configure SIEM to cross-reference cloud, network, and application logs", "Implement unified correlation rules", "Verify cross-source correlation accuracy"],
        4: ["Centralize audit review in single SIEM platform", "Configure unified dashboards for audit analysis", "Implement role-based access to audit review tools", "Standardize review procedures across teams"],
        5: ["Integrate audit analysis with vulnerability, asset, and threat data", "Configure SIEM enrichment with context sources", "Implement automated risk scoring for audit events", "Review integrated analysis results weekly"],
        6: ["Correlate logical audit events with physical access records", "Integrate badge reader logs with SIEM", "Alert on logical access without corresponding physical presence", "Review correlation findings weekly"],
        7: ["Define permitted actions after audit review findings", "Implement automated response playbooks", "Configure approved remediation actions", "Track audit finding resolution"],
        8: ["Enable full command logging for privileged sessions", "Configure session recording for administrative access", "Implement command analysis for security violations", "Review privileged command logs weekly"],
        9: ["Correlate cyber audit logs with physical access monitoring", "Integrate video surveillance with logical access events", "Alert on suspicious physical-cyber correlations", "Review correlation findings weekly"],
        10: ["Implement adjustable audit logging levels", "Configure dynamic audit level increases during incidents", "Enable granular audit control per system component", "Document audit level adjustment procedures"],
    }
    CONTROLS[f"AU-6({enh})"] = {
        "summary": names[enh],
...
for enh in range(1, 8):
    ...
    steps_map = {
        1: ["Implement write-once storage for critical audit logs", "Configure WORM storage policies", "Verify write-once enforcement", "Test tamper resistance"],
        2: ["Store audit logs on dedicated logging infrastructure", "Implement cross-account or cross-subscription logging", "Verify physical or logical separation of log storage", "Monitor log storage infrastructure independently"],
        3: ["Enable encryption at rest for all audit logs", "Implement log signing for integrity verification", "Configure KMS key policies for audit log encryption", "Verify cryptographic protection regularly"],
        4: ["Restrict audit log access to security team members only", "Implement IAM policies limiting audit log access", "Configure separate roles for audit log administration", "Review audit log access quarterly"],
        5: ["Require dual authorization for audit log deletion or movement", "Implement two-person integrity for log management actions", "Log all audit log management operations", "Test dual authorization enforcement"],
        6: ["Configure read-only access to audit records for reviewers", "Prevent modification of audit records after creation", "Implement append-only log storage", "Verify read-only enforcement"],
        7: ["Store audit logs on systems with different operating system", "Implement cross-platform log storage for defense in depth", "Verify OS diversity in log storage infrastructure", "Document OS diversity configuration"],
    }
    CONTROLS[f"AU-9({enh})"] = {
        "summary": names[enh],
        "remediation_steps": steps_map[enh],
...
for enh in range(1, 6):
    names = {1: "Association of identities", 2: "Validate binding of information producer identity", 3: "Chain of custody", 4: "Validate binding of information reviewer identity", 5: "Digital signatures"}
    steps_list = {
        1: ["Bind user identities to audit records cryptographically", "Implement digital certificate-based authentication", "Verify identity binding in audit record integrity checks", "Test non-repudiation mechanisms"],
        2: ["Validate information producer identity at creation", "Implement code signing for software artifacts", "Configure content attribution mechanisms", "Verify producer identity binding"],
        3: ["Implement chain of custody tracking for evidence", "Configure hash-chaining for audit records", "Maintain cryptographic proof of custody transfers", "Verify chain of custody integrity"],
        4: ["Bind reviewer identity to review actions", "Implement approval signatures in workflows", "Log reviewer identity for all review actions", "Verify reviewer identity binding"],
        5: ["Implement digital signatures for non-repudiation", "Configure PKI for document and transaction signing", "Verify digital signature validity", "Manage signing certificates and keys"],
    }
...
for enh, summ, steps in [
    ...
    ("2", "Standardized formats for audit record content", [
...
for enh, summ, steps in [
    ("1", "System start-up audit capture", [
    ...
    ("3", "Remote viewing and listening", [
...
CONTROLS["CA-1"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: CA-1 Assessment Procedures", "NIST SP 800-37: RMF Guide"],
    "evidence_types": ["policy_document", "policy_review_date"],
}
...
CONTROLS["CA-2"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: CA-2 Assessment Procedures", "NIST SP 800-53A: Assessment Guide"],
...
for enh, summ, steps in [
    ...
    ("3", "Leveraging results from external organizations", [
...
for enh in range(1, 8):
    names = {1: "Unclassified national security system connections", 2: "Classified national security system connections", 3: "Unclassified non-national security system connections", 4: "Connections to public networks", 5: "Restrictions on external system connections", 6: "Transfer authorizations", 7: "Transitive information exchanges"}
    CONTROLS[f"CA-3({enh})"] = {
        "summary": names[enh],
        "remediation_steps": [
...
CONTROLS["CA-4"] = {
    ...
    "remediation_steps": ["This control has been withdrawn and incorporated into CA-2", "Verify CA-2 assessment controls are implemented"],
...
CONTROLS["CA-6"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: CA-6 Assessment Procedures", "NIST SP 800-37: RMF Guide"],
...
CONTROLS["CA-7"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: CA-7 Assessment Procedures", "NIST SP 800-137: ISCM Guide"],
...
for enh, summ, steps in [
    ...
    ("6", "Automation support for monitoring", [
...
CONTROLS["CA-8"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: CA-8 Assessment Procedures", "NIST SP 800-115: Technical Guide to Information Security Testing"],
    "evidence_types": ["pentest_reports"],
}

for enh, summ, steps in [
    ("1", "Independent penetration testing agent", [
...
CONTROLS["CM-2"] = {
    ...
    "recommended_reading": ["NIST SP 800-53A Rev 5: CM-2 Assessment Procedures", "CIS Benchmarks for AWS/Azure/GCP"],
    "evidence_types": ["config_rules", "config_snapshots"],
}

for enh, summ, steps in [
    ("1", "Reviews and updates to baseline configurations", [
    ...
    ("4", "Unauthorized software (withdrawn)", [
    ...
    ("5", "Authorized software (withdrawn)", [
        "This enhancement has been withdrawn and incorporated into CM-7(4) and CM-7(5)",
        "Verify CM-7 software restriction controls are implemented",
    ]),
    ...
    ("7", "Configure systems and components for high-risk areas", [
...
for enh, summ, steps in [
    ("1", "Automated documentation and notification", [
    ...
    ("2", "Testing and validation of changes", [
    ...
    ("5", "Automated security response to changes", [
    ...
    ("8", "Prevent or restrict configuration changes", [
...
for enh in range(1, 8):
    names = {1: "Automated access enforcement and audit records", 2: "Review system changes", 3: "Signed components", 4: "Dual authorization", 5: "Privilege limitation for production and operation", 6: "Limit library privileges", 7: "Automatic implementation of security safeguards"}
    steps_map = {
        1: ["Implement automated access enforcement for change management", "Configure audit logging for all change activities", "Generate automated reports on change access", "Verify access enforcement effectiveness"],
        2: ["Conduct post-implementation review of system changes", "Verify changes match approved specifications", "Document review findings", "Track unresolved review findings"],
        3: ["Require code signing for all deployed components", "Implement artifact signing in CI/CD pipeline", "Verify signatures before deployment", "Manage signing keys securely"],
        4: ["Require dual authorization for production changes", "Implement two-person review in CI/CD pipeline", "Log dual authorization for audit", "Test dual authorization enforcement"],
        5: ["Restrict production access to deployment pipelines", "Prohibit direct production access except for emergency", "Implement break-glass procedures for emergency production access", "Audit production access monthly"],
        6: ["Restrict access to software libraries and repositories", "Implement library access controls based on role", "Monitor library access and changes", "Review library privileges quarterly"],
        7: ["Implement automated security controls triggered by configuration changes", "Configure automated rollback on security violations", "Enable automated compliance scanning after changes", "Verify automatic safeguard effectiveness"],
    }
    CONTROLS[f"CM-5({enh})"] = {
        "summary": names[enh],
        "remediation_steps": steps_map[enh],
...
for enh, summ, steps in [
    ("1", "Automated management and enforcement", [
...
for enh in range(1, 10):
    names = {1: "Periodic review", 2: "Prevent program execution", 3: "Registration compliance", 4: "Unauthorized software or deny-all/allow-by-exception", 5: "Authorized software or allow-all/deny-by-exception", 6: "Confined environments with limited privileges", 7: "Code execution in protected environments", 8: "Binary or machine executable code", 9: "Prohibiting use of unauthorized hardware"}
    steps_map = {
        1: ["Conduct periodic review of enabled system capabilities", "Disable functions no longer needed", "Document review results and actions taken", "Review at least annually"],
        2: ["Implement application whitelisting to prevent unauthorized execution", "Configure AppLocker or equivalent program execution controls", "Block execution of unauthorized scripts and binaries", "Monitor blocked execution attempts"],
        3: ["Register software in organizational software inventory", "Verify software registration compliance", "Remove unregistered software", "Update registration requirements annually"],
        4: ["Implement deny-all application whitelisting policy", "Configure explicit allow list for authorized software", "Block installation of unauthorized software", "Review authorized software list quarterly"],
        5: ["Maintain deny list of prohibited software", "Configure automated detection and removal of prohibited software", "Alert on prohibited software installation attempts", "Update deny list based on threat intelligence"],
        6: ["Execute software in sandboxed or confined environments", "Configure container security policies with restricted capabilities", "Implement mandatory access control for application processes", "Verify confinement effectiveness through testing"],
        7: ["Execute untrusted code in protected environments only", "Configure separate execution environments for untrusted code", "Implement code isolation mechanisms", "Monitor protected environment activity"],
        8: ["Restrict execution of binary or machine-executable code", "Require approval for binary code execution", "Implement code signing verification", "Block unsigned binary execution"],
        9: ["Maintain inventory of authorized hardware", "Implement network access control for hardware", "Block unauthorized hardware connections", "Detect and alert on unauthorized hardware"],
    }
    CONTROLS[f"CM-7({enh})"] = {
        "summary": names[enh],
        "remediation_steps": steps_map[enh],
...
for enh in range(1, 10):
    names = {1: "Updates during installation and removal", 2: "Automated maintenance", 3: "Automated unauthorized component detection", 4: "Accountability information", 5: "No duplicate accounting of components", 6: "Assessed configurations and approved deviations", 7: "Centralized repository", 8: "Automated location tracking", 9: "Assignment of components to systems"}
    steps_map = {
        1: ["Update inventory when installing or removing components", "Configure automated inventory updates on deployment", "Verify inventory accuracy after changes", "Alert on inventory discrepancies"],
        2: ["Implement automated inventory maintenance through discovery tools", "Configure scheduled inventory scans", "Enable auto-update of inventory on resource changes", "Verify automated maintenance completeness"],
        3: ["Configure automated detection of unauthorized components", "Alert on components not in approved inventory", "Implement network-based asset discovery", "Investigate unauthorized components within 24 hours"],
        4: ["Include accountability information in inventory (owner, custodian, location)", "Assign ownership to all inventory items", "Update accountability information on personnel changes", "Review accountability assignments quarterly"],
        5: ["Implement unique identifiers for all components", "Prevent duplicate entries in inventory", "Reconcile inventory across data sources", "Verify unique identification accuracy"],
        6: ["Document assessed configurations for inventory items", "Track approved configuration deviations", "Review deviations and re-assess periodically", "Ensure baseline compliance for all components"],
        7: ["Maintain centralized inventory repository", "Configure all discovery tools to feed central CMDB", "Provide role-based access to inventory data", "Backup inventory data regularly"],
        8: ["Implement automated tracking of component locations", "Configure cloud provider resource tagging for location", "Track physical asset locations via asset management", "Verify location accuracy quarterly"],
        9: ["Map components to system boundaries", "Ensure each component is assigned to exactly one system", "Update assignments on system boundary changes", "Verify assignments during authorization"],
    }
    CONTROLS[f"CM-8({enh})"] = {
        "summary": names[enh],
        "remediation_steps": steps_map[enh],
...
for ctrl in ["CM-11", "CM-12", "CM-13", "CM-14"]:
    ...
    names = {"11": "User-installed software", "12": "Information location", "13": "Data action mapping", "14": "Signed components"}
    steps_map = {
        "11": ["Define policy for user-installed software", "Configure application control to restrict user installs", "Monitor for unauthorized user software installations", "Review user-installed software policy annually"],
        "12": ["Document locations of sensitive information across systems", "Implement automated data discovery and classification", "Maintain data flow maps for all sensitive information", "Update information location records on system changes"],
        "13": ["Map data actions (collection, storage, sharing, processing) for PII", "Document data action purposes and legal bases", "Implement data action tracking in privacy management tool", "Review data action mappings annually"],
        "14": ["Implement component signing and verification", "Require signed components in deployment pipeline", "Verify component signatures before installation", "Manage signing infrastructure securely"],
    }
    CONTROLS[ctrl] = {
...
CP_BASE = {
    "CP-1": ("Develop, document, and disseminate contingency planning policy and procedures", ["Draft contingency planning policy", "Define roles and responsibilities for contingency operations", "Establish contingency plan review cycle (at least annually)", "Distribute policy to all organizational personnel"], "Confluence > SEC Space > Contingency Planning Policy", ["policy_document", "policy_review_date"]),
    "CP-2": ("Develop a contingency plan for the system", ["Develop system contingency plan with recovery objectives (RPO/RTO)", "Identify essential missions and business functions", "Define recovery strategies and procedures", "Distribute plan to key personnel"], "Confluence > SEC Space > Contingency Plan", ["policy_document", "contingency_plan"]),
    "CP-3": ("Provide contingency training to system users", ["Conduct initial contingency training for new personnel", "Provide annual contingency training refresher", "Include role-specific contingency responsibilities", "Document training completion"], "KnowBe4 > Training > Contingency Modules", ["training_records"]),
    "CP-4": ("Test the contingency plan to determine effectiveness", ["Conduct annual contingency plan testing", "Include tabletop and functional exercises", "Document test results and lessons learned", "Update contingency plan based on test findings"], "GRC Platform > Contingency Tests", ["test_reports"]),
    "CP-5": ("Contingency plan update (withdrawn - incorporated into CP-2)", ["This control has been withdrawn and incorporated into CP-2", "Verify CP-2 contingency planning controls are implemented"], "See CP-2", ["policy_document"]),
    "CP-6": ("Establish an alternate storage site", ["Identify and establish alternate storage site", "Configure geographic separation for alternate storage", "Implement encryption for data at alternate site", "Test data retrieval from alternate site annually"], "AWS > S3 Cross-Region Replication | Azure > GRS", ["config_snapshots"]),
    "CP-7": ("Establish an alternate processing site", ["Identify and establish alternate processing site", "Configure multi-region or multi-zone deployment", "Verify alternate site can support essential operations", "Test failover to alternate processing site annually"], "AWS > Multi-Region | Azure > Paired Regions", ["config_snapshots"]),
    "CP-8": ("Establish alternate telecommunications services", ["Identify alternate telecommunications providers", "Configure redundant network connectivity", "Test alternate telecommunications paths", "Document telecommunications recovery procedures"], "Network > Redundant Connectivity", ["config_snapshots"]),
    "CP-9": ("Conduct system backups", ["Configure automated backups for all critical systems", "Implement backup encryption at rest", "Verify backup integrity through regular restore tests", "Monitor backup job success and alert on failures"], "AWS Backup > Plans | Azure Backup > Vaults", ["backup_reports", "config_snapshots"]),
    "CP-10": ("Provide for system recovery and reconstitution", ["Document system recovery procedures", "Implement automated recovery mechanisms", "Test system recovery annually", "Verify recovery meets defined RPO and RTO"], "AWS > Disaster Recovery | Azure > Site Recovery", ["backup_reports", "test_reports"]),
    "CP-11": ("Provide alternate communications protocols", ["Identify alternate communications protocols", "Configure failover communications", "Test alternate protocol activation", "Document communications recovery procedures"], "Network > Alternate Protocols", ["config_snapshots"]),
    "CP-12": ("Safe mode operation", ["Define safe mode operating conditions", "Implement safe mode activation procedures", "Test safe mode functionality", "Document safe mode limitations and capabilities"], "Systems > Safe Mode Configuration", ["config_snapshots"]),
    "CP-13": ("Alternate security mechanisms", ["Identify alternate security mechanisms for primary failure", "Document failover security procedures", "Test alternate security mechanism activation", "Verify alternate mechanisms provide adequate protection"], "Security > Failover Mechanisms", ["config_snapshots"]),
}
...
# CP-2 enhancements
for i in range(1, 9):
    names = {1: "Coordinate with related plans", 2: "Capacity planning", 3: "Resume mission functions", 4: "Resume all mission functions", 5: "Continue mission functions", 6: "Alternate processing and storage site", 7: "Coordinate with external service providers", 8: "Identify critical assets"}
...
# CP-4 enhancements
for i in range(1, 6):
    names = {1: "Coordinate with related plans testing", 2: "Alternate processing site testing", 3: "Automated testing", 4: "Full recovery and reconstitution testing", 5: "Self-challenge testing"}
    CP_ENH[f"CP-4({i})"] = (names[i], "Contingency Planning > Testing")

# CP-6 enhancements
for i in range(1, 4):
    names = {1: "Separation from primary site", 2: "Recovery time and point objectives", 3: "Accessibility"}
    CP_ENH[f"CP-6({i})"] = (names[i], "Alternate Storage > Configuration")

# CP-7 enhancements
for i in range(1, 7):
    names = {1: "Separation from primary site", 2: "Accessibility", 3: "Priority of service", 4: "Preparation for use", 5: "Equivalent information security safeguards", 6: "Inability to return to primary site"}
    CP_ENH[f"CP-7({i})"] = (names[i], "Alternate Processing > Configuration")

# CP-8 enhancements
for i in range(1, 6):
    names = {1: "Priority of service provisions", 2: "Single points of failure", 3: "Separation of primary and alternate providers", 4: "Provider contingency plan", 5: "Alternate telecommunication service testing"}
    CP_ENH[f"CP-8({i})"] = (names[i], "Telecommunications > Redundancy")

# CP-9 enhancements
for i in range(1, 9):
    names = {1: "Testing for reliability and integrity", 2: "Test restoration using sampling", 3: "Separate storage for critical information", 4: "Protection from unauthorized modification", 5: "Transfer to alternate storage site", 6: "Redundant secondary system", 7: "Dual authorization for deletion or destruction", 8: "Cryptographic protection"}
    CP_ENH[f"CP-9({i})"] = (names[i], "Backup > Configuration")

# CP-10 enhancements
for i in range(1, 7):
    names = {1: "Contingency plan testing (withdrawn)", 2: "Transaction recovery", 3: "Compensating security controls (withdrawn)", 4: "Restore within time period", 5: "Failover capability", 6: "Component protection"}
...
IA_BASE = {
    "IA-1": ("Develop, document, and disseminate identification and authentication policy", ["Draft identification and authentication policy", "Define authentication requirements by system type and risk level", "Establish authenticator management procedures", "Review and update policy annually"], "Confluence > SEC Space > IA Policy", ["policy_document", "policy_review_date"]),
    "IA-2": ("Uniquely identify and authenticate organizational users", ["Enable MFA for all interactive user accounts", "Configure identity provider with unique user identifiers", "Implement phishing-resistant MFA (FIDO2/WebAuthn)", "Verify MFA enrollment coverage for all users"], "Okta > Security > Multifactor | Entra ID > MFA", ["iam_credential_report", "okta_users", "entra_users"]),
    "IA-3": ("Authenticate devices before establishing connections", ["Implement 802.1X network access control for device authentication", "Configure certificate-based device authentication", "Deploy NAC solution for device posture assessment", "Block unauthenticated devices from network access"], "NAC > Device Authentication | Intune > Compliance", ["config_snapshots", "intune_devices"]),
    "IA-4": ("Manage system identifiers for users, processes, and devices", ["Implement unique identifier assignment process", "Prevent identifier reuse for defined period", "Disable identifiers after period of inactivity", "Maintain identifier lifecycle management"], "IAM > Users | Entra ID > User Management", ["iam_users", "entra_users"]),
    "IA-5": ("Manage system authenticators", ["Enforce password complexity (min 12 chars, mixed case, numbers, symbols)", "Implement password history (last 24 passwords)", "Configure password maximum age (90 days) or use passwordless", "Protect authenticator content from unauthorized disclosure"], "IAM > Account Settings > Password Policy | Okta > Security > Authentication", ["iam_credential_report", "okta_policies"]),
    "IA-6": ("Obscure authenticator feedback during authentication", ["Configure authentication interfaces to mask password entry", "Verify password masking on all login screens", "Test authenticator feedback obscuring", "Ensure credential display protection across all platforms"], "Application > Authentication UI", ["config_snapshots"]),
    "IA-7": ("Authenticate using cryptographic mechanisms to approved standards", ["Implement FIPS 140-2 validated cryptographic modules", "Configure TLS 1.2+ for all authentication flows", "Use approved algorithms for authenticator protection", "Verify cryptographic module compliance"], "KMS > Key Configuration | TLS > Settings", ["config_snapshots"]),
    "IA-8": ("Authenticate non-organizational users", ["Configure authentication for external users and partners", "Implement B2B federation for partner access", "Require MFA for non-organizational users", "Monitor non-organizational user access"], "Entra ID > External Identities | Okta > External IdP", ["entra_users", "config_snapshots"]),
    "IA-9": ("Identify and authenticate services", ["Implement service-to-service authentication (mTLS, API keys, OAuth)", "Configure service mesh with mutual TLS", "Verify service identity before accepting connections", "Rotate service credentials regularly"], "Service Mesh > mTLS | API Gateway > Service Auth", ["config_snapshots"]),
    "IA-10": ("Adaptive identification and authentication", ["Implement risk-based authentication policies", "Configure adaptive MFA based on context (location, device, behavior)", "Set up step-up authentication for high-risk activities", "Monitor adaptive authentication decisions"], "Okta > Policies > Adaptive MFA | Entra ID > Conditional Access", ["okta_policies", "entra_conditional_access_policies"]),
    "IA-11": ("Re-authentication", ["Configure re-authentication for privileged operations", "Set session re-authentication intervals", "Implement step-up authentication for sensitive transactions", "Verify re-authentication enforcement"], "Application > Session > Re-Authentication", ["config_snapshots"]),
    "IA-12": ("Identity proofing", ["Implement identity proofing process for user registration", "Verify identity using government-issued ID or equivalent", "Maintain identity proofing records", "Review identity proofing procedures annually"], "HR > Identity Verification | Okta > Registration", ["identity_proofing_records"]),
    "IA-13": ("Identity providers and authorization servers", ["Deploy enterprise identity provider (Okta, Entra ID)", "Configure OAuth 2.0/OIDC authorization server", "Implement token management and revocation", "Monitor identity provider health and security"], "Okta > Applications > Auth Server | Entra ID > App Registrations", ["config_snapshots"]),
}
...
for enh_num, summ in IA2_ENH.items():
    CONTROLS[f"IA-2({enh_num})"] = {
        "summary": summ,
        "remediation_steps": [
            f"Implement {summ.lower()} controls" if "withdrawn" not in summ.lower() else f"This enhancement has been withdrawn",
            "Configure MFA policies in identity provider" if "withdrawn" not in summ.lower() else "Verify base control IA-2 is implemented",
            "Verify enforcement across all access methods" if "withdrawn" not in summ.lower() else "No additional action required",
            "Test authentication controls" if "withdrawn" not in summ.lower() else "Document withdrawal acknowledgment",
        ],
...
for ctrl_base, max_enh, names_dict in [
    ("IA-3", 4, {1: "Cryptographic bidirectional authentication", 2: "Cryptographic bidirectional network authentication", 3: "Dynamic address allocation", 4: "Attestation"}),
    ("IA-4", 9, {1: "Prohibit account identifiers as public identifiers", 2: "Supervisor authorization", 3: "Multiple forms of certification", 4: "Identify user status", 5: "Dynamic management", 6: "Cross-organization management", 7: "In-person registration", 8: "Pairwise pseudonymous identifiers", 9: "Attribute maintenance and protection"}),
    ("IA-5", 18, {1: "Password-based authentication", 2: "PKI-based authentication", 3: "In-person or trusted third-party registration", 4: "Automated support for password strength", 5: "Change authenticators prior to delivery", 6: "Protection of authenticators", 7: "No embedded unencrypted static authenticators", 8: "Multiple system accounts", 9: "Federated credential management", 10: "Dynamic credential binding", 11: "Hardware token-based authentication", 12: "Biometric authentication", 13: "Expiration of cached authenticators", 14: "Managing content of PKI trust stores", 15: "GSA-approved products and services", 16: "In-person or trusted third-party issuer", 17: "Presentation attack detection for biometric authenticators", 18: "Password managers"}),
    ("IA-6", 1, {1: "Authentication feedback methods"}),
    ("IA-7", 2, {1: "Hardware-based cryptographic authentication", 2: "Software-based cryptographic authentication"}),
    ("IA-8", 6, {1: "Acceptance of PIV credentials from other agencies", 2: "Acceptance of third-party credentials", 3: "Use of FICAM-approved products", 4: "Use of defined profiles", 5: "Acceptance of PIV-I credentials", 6: "Disassociability"}),
    ...
    ("IA-12", 6, {1: "Supervisor authorization", 2: "Identity evidence", 3: "Identity evidence validation and verification", 4: "In-person validation and verification", 5: "Address confirmation", 6: "Accept externally-proofed identities"}),
]:
    for enh_num in range(1, max_enh + 1):
        ...
        if ctrl_enh_id not in CONTROLS:
            ...
            CONTROLS[ctrl_enh_id] = {
                "summary": summ,
                "remediation_steps": [
                    f"Implement {summ.lower()}" if "withdrawn" not in summ.lower() else "This enhancement has been withdrawn",
                ...
                "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_enh_id} Assessment Procedures"],
                "evidence_types": ["config_snapshots"],
            }
...
IR_BASE = {
    "IR-1": ("Develop, document, and disseminate incident response policy", ["Draft incident response policy and procedures", "Define incident categories and severity levels", "Establish incident response team roles and responsibilities", "Review and update policy annually"], "Confluence > SEC Space > Incident Response Policy", ["policy_document", "policy_review_date"]),
    "IR-2": ("Provide incident response training", ["Conduct initial incident response training for IR team", "Provide annual refresher training", "Include role-specific IR training content", "Document training completion"], "KnowBe4 > Training > Incident Response", ["training_records"]),
    "IR-3": ("Test the incident response capability", ["Conduct annual incident response exercises", "Include tabletop and simulated incident exercises", "Document exercise results and lessons learned", "Update IR plan based on exercise findings"], "GRC Platform > IR Exercises", ["test_reports"]),
    "IR-4": ("Implement an incident handling capability", ["Establish incident response process (detect, analyze, contain, eradicate, recover)", "Configure SIEM/SOAR for incident handling workflows", "Define escalation procedures and communication plans", "Track and close incidents with documented resolution"], "Sentinel > Incidents | PagerDuty > Incidents", ["siem_alerts", "incident_records"]),
    "IR-5": ("Track and document incidents", ["Implement incident tracking system", "Record all incidents with required details", "Maintain incident database with classification and resolution", "Generate incident metrics and reports monthly"], "SIEM > Incident Tracking | ServiceNow > Security Incidents", ["incident_records", "siem_alerts"]),
    "IR-6": ("Report incidents to appropriate authorities", ["Define incident reporting requirements and thresholds", "Establish external reporting procedures (CISA, law enforcement)", "Configure automated reporting for qualifying incidents", "Document reporting timelines and contacts"], "Incident Response > Reporting Procedures", ["incident_records"]),
    "IR-7": ("Provide incident response assistance", ["Establish incident response support resources", "Configure help desk for incident reporting", "Provide incident response guidance to all users", "Maintain incident response contact list"], "ServiceNow > Incident Support | Help Desk", ["incident_records"]),
    "IR-8": ("Develop incident response plan", ["Document comprehensive incident response plan", "Define incident categories, severity, and response procedures", "Identify external resources and coordination points", "Review and update plan annually"], "Confluence > SEC Space > IR Plan", ["policy_document"]),
    "IR-9": ("Information spillage response", ["Define information spillage response procedures", "Identify responsible personnel for spillage incidents", "Implement containment procedures for data spillage", "Document spillage incident handling and resolution"], "Incident Response > Spillage Procedures", ["policy_document"]),
    "IR-10": ("Integrated information security analysis team", ["Establish integrated security analysis team", "Define team composition and responsibilities", "Provide team with necessary tools and access", "Review team effectiveness annually"], "Security Operations > Analysis Team", ["policy_document"]),
}
...
IR_ENH_DEFS = {
    ...
    "IR-2(1)": "Simulated events in IR training", "IR-2(2)": "Automated IR training environments", "IR-2(3)": "Breach response training",
    "IR-3(1)": "Automated IR testing", "IR-3(2)": "Coordination with related plans testing", "IR-3(3)": "Continuous improvement from IR testing",
    "IR-4(1)": "Automated incident handling processes", "IR-4(2)": "Dynamic reconfiguration", "IR-4(3)": "Continuity of operations",
    "IR-4(4)": "Information correlation", "IR-4(5)": "Automatic disable of system", "IR-4(6)": "Insider threats",
    "IR-4(7)": "Insider threats — intra-organization coordination", "IR-4(8)": "Correlation with external organizations",
    "IR-4(9)": "Dynamic response capability", "IR-4(10)": "Supply chain coordination",
    "IR-4(11)": "Integrated incident response team", "IR-4(12)": "Malicious code and forensic analysis",
    "IR-4(13)": "Behavior analysis", "IR-4(14)": "Security operations center",
    "IR-5(1)": "Automated incident tracking and reporting",
    "IR-6(1)": "Automated reporting", "IR-6(2)": "Vulnerabilities related to incidents", "IR-6(3)": "Supply chain coordination",
    "IR-7(1)": "Automation support for availability of information and support", "IR-7(2)": "Coordination with external providers",
    "IR-8(1)": "Breaches",
    "IR-9(1)": "Responsible personnel for spillage response", "IR-9(2)": "Training for spillage response",
    "IR-9(3)": "Post-spillage operations", "IR-9(4)": "Exposure to unauthorized personnel",
...
# MA — Maintenance
MA_DEFS = {
    "MA-1": ("Develop, document, and disseminate maintenance policy and procedures", ["Draft system maintenance policy", "Define maintenance roles and responsibilities", "Establish maintenance scheduling requirements", "Review and update policy annually"], "Confluence > SEC Space > Maintenance Policy", ["policy_document"]),
    "MA-2": ("Schedule, perform, document, and review maintenance on system components", ["Implement preventive maintenance schedule for all components", "Document all maintenance activities", "Review maintenance records for completeness", "Verify maintenance does not introduce vulnerabilities"], "ServiceNow > Change Management > Maintenance", ["maintenance_records"]),
    "MA-3": ("Approve, control, and monitor maintenance tools", ["Maintain approved maintenance tools inventory", "Inspect maintenance tools before use", "Monitor maintenance tool usage", "Remove unauthorized maintenance tools"], "CMDB > Maintenance Tools", ["asset_inventory"]),
    "MA-4": ("Approve and monitor nonlocal maintenance activities", ["Implement secure remote maintenance access", "Require MFA for remote maintenance sessions", "Log and monitor all remote maintenance activities", "Terminate remote maintenance sessions when complete"], "PAM > Remote Maintenance | VPN > Maintenance Access", ["audit_logs"]),
    "MA-5": ("Establish a process for maintenance personnel authorization", ["Verify maintenance personnel security clearance", "Escort unauthorized maintenance personnel", "Supervise maintenance activities by non-cleared personnel", "Document maintenance personnel authorization"], "HR > Contractor Verification | Badge System", ["personnel_records"]),
    "MA-6": ("Obtain maintenance support and spare parts within defined time period", ["Maintain spare parts inventory for critical components", "Establish maintenance support agreements (SLAs)", "Track spare parts availability", "Review maintenance support agreements annually"], "ServiceNow > Asset Management > Spare Parts", ["maintenance_records"]),
    "MA-7": ("Field maintenance", ["Define field maintenance procedures and restrictions", "Implement security controls for field maintenance", "Verify field maintenance does not compromise security", "Document field maintenance activities"], "ServiceNow > Field Service > Maintenance", ["maintenance_records"]),
}
...
# MP — Media Protection
MP_DEFS = {
    "MP-1": ("Develop, document, and disseminate media protection policy and procedures", ["Draft media protection policy", "Define media handling and classification requirements", "Establish media disposal procedures", "Review and update policy annually"], "Confluence > SEC Space > Media Protection Policy", ["policy_document"]),
    "MP-2": ("Restrict access to digital and non-digital media", ["Implement physical access controls for media storage", "Restrict digital media access through encryption and access controls", "Document media access authorization", "Review media access controls quarterly"], "Physical Security > Media Storage", ["config_snapshots"]),
    "MP-3": ("Mark media with applicable security markings", ["Implement media labeling procedures", "Apply classification markings to all media", "Verify marking accuracy during handling", "Train personnel on media marking requirements"], "Data Classification > Labeling", ["policy_document"]),
    "MP-4": ("Physically control and securely store media", ["Store media in controlled areas with physical access controls", "Implement environmental protections for media storage", "Track media location and custody", "Review media storage controls annually"], "Physical Security > Media Storage Facility", ["config_snapshots"]),
    "MP-5": ("Protect and control media during transport", ["Implement media transport security procedures", "Use encrypted containers for media in transit", "Track media during transport", "Verify media integrity upon receipt"], "Logistics > Secure Transport", ["policy_document"]),
    "MP-6": ("Sanitize media before disposal or reuse", ["Implement media sanitization procedures (NIST SP 800-88)", "Use approved sanitization tools and methods", "Verify sanitization completeness", "Document sanitization activities and maintain certificates"], "IT Operations > Media Sanitization", ["sanitization_records"]),
    "MP-7": ("Restrict use of portable storage devices", ["Define portable storage device usage policy", "Implement USB device control through endpoint protection", "Block unauthorized portable storage devices", "Monitor portable storage device usage"], "Intune > Device Configuration > USB Restrictions", ["intune_devices"]),
    "MP-8": ("Media downgrading", ["Define media downgrading procedures", "Implement approved downgrading methods", "Verify downgrading completeness", "Document downgrading activities"], "Data Classification > Downgrading", ["policy_document"]),
}
...
PE_DEFS = {
    "PE-1": ("Develop, document, and disseminate physical and environmental protection policy", ["Draft physical and environmental protection policy", "Define facility access requirements", "Establish environmental control requirements", "Review and update policy annually"], "Confluence > SEC Space > Physical Security Policy", ["policy_document"]),
    "PE-2": ("Develop, approve, and maintain physical access authorizations", ["Maintain list of authorized personnel for facility access", "Issue access credentials (badges) to authorized personnel", "Review and update access authorizations quarterly", "Remove access authorizations upon termination"], "Badge System > Access Authorizations", ["badge_records"]),
    "PE-3": ("Enforce physical access authorizations at facility entry points", ["Implement badge readers and access control at all entry points", "Deploy security guards or video surveillance at entry points", "Log all physical access events", "Monitor access logs for anomalies"], "Badge System > Access Control | CCTV > Monitoring", ["badge_records", "cctv_logs"]),
    "PE-4": ("Control physical access to information system distribution and transmission lines", ["Protect network cabling and distribution points", "Restrict access to server rooms and network closets", "Monitor access to distribution infrastructure", "Inspect physical infrastructure regularly"], "Facilities > Network Infrastructure Access", ["badge_records"]),
    "PE-5": ("Control physical access to information system output devices", ["Restrict access to printers, screens, and output devices", "Implement follow-me printing", "Position displays to prevent shoulder surfing", "Secure output device locations"], "Facilities > Output Device Security", ["config_snapshots"]),
    "PE-6": ("Monitor physical access to the facility", ["Implement physical access monitoring and logging", "Review physical access logs regularly", "Investigate unauthorized access attempts", "Maintain monitoring system availability"], "Badge System > Access Logs | CCTV > Live Monitoring", ["badge_records", "cctv_logs"]),
    "PE-7": ("Control physical access by visitors (withdrawn - incorporated into PE-2 and PE-3)", ["This control has been withdrawn", "Verify PE-2 and PE-3 visitor controls are implemented"], "See PE-2 and PE-3", ["policy_document"]),
    "PE-8": ("Maintain visitor access records", ["Log all visitor entries and exits", "Require visitor escort in controlled areas", "Retain visitor logs per retention policy", "Review visitor logs weekly"], "Badge System > Visitor Management", ["visitor_logs"]),
    "PE-9": ("Protect power equipment and power cabling", ["Install physical protections for power infrastructure", "Restrict access to power equipment", "Implement redundant power supplies (UPS)", "Inspect power infrastructure regularly"], "Facilities > Power Infrastructure", ["maintenance_records"]),
    "PE-10": ("Provide emergency shutoff capability", ["Install emergency power shutoff switches", "Label and test emergency shutoffs quarterly", "Document emergency shutoff procedures", "Train personnel on emergency shutoff use"], "Facilities > Emergency Power Controls", ["test_reports"]),
    "PE-11": ("Provide uninterruptible power supply", ["Deploy UPS for critical systems", "Configure UPS monitoring and alerting", "Test UPS failover quarterly", "Maintain UPS batteries per manufacturer schedule"], "Facilities > UPS > Configuration", ["maintenance_records"]),
    "PE-12": ("Provide emergency lighting", ["Install emergency lighting in all occupied areas", "Test emergency lighting monthly", "Maintain emergency lighting batteries", "Document emergency lighting coverage"], "Facilities > Emergency Lighting", ["maintenance_records"]),
    "PE-13": ("Protect the facility from fire", ["Install fire detection and suppression systems", "Maintain fire suppression equipment per code", "Conduct fire drills annually", "Document fire protection system maintenance"], "Facilities > Fire Protection", ["maintenance_records"]),
    "PE-14": ("Maintain temperature and humidity controls", ["Configure HVAC for appropriate temperature and humidity", "Implement temperature and humidity monitoring with alerts", "Set acceptable ranges per equipment specifications", "Document environmental monitoring records"], "Facilities > Environmental Monitoring > HVAC", ["environmental_logs"]),
    "PE-15": ("Protect against water damage", ["Implement water detection sensors in server rooms", "Configure water detection alerts", "Install shutoff valves for water sources near equipment", "Document water damage prevention measures"], "Facilities > Water Detection", ["environmental_logs"]),
    "PE-16": ("Control delivery and removal of system components", ["Implement delivery and receiving procedures", "Inspect deliveries for tampering", "Log all component deliveries and removals", "Restrict delivery access to designated areas"], "Facilities > Loading Dock > Security", ["asset_inventory"]),
    "PE-17": ("Alternate work site security", ["Define security requirements for alternate work sites", "Verify security controls at alternate sites", "Provide VPN and encryption for remote work", "Review alternate work site security annually"], "IT > Remote Work > Security Requirements", ["config_snapshots"]),
    "PE-18": ("Location of system components", ["Position systems to minimize physical threat exposure", "Locate critical systems away from windows and external walls", "Document component placement rationale", "Review component locations during facility changes"], "Facilities > Data Center > Layout", ["config_snapshots"]),
    "PE-19": ("Information leakage through electromagnetic emanations", ["Implement TEMPEST or electromagnetic shielding where required", "Position equipment to minimize emanation exposure", "Test for electromagnetic leakage", "Document emanation protection measures"], "Facilities > TEMPEST > Shielding", ["config_snapshots"]),
    "PE-20": ("Asset monitoring and tracking", ["Implement RFID or barcode tracking for physical assets", "Configure automated asset location tracking", "Alert on unauthorized asset movement", "Reconcile physical asset inventory quarterly"], "Asset Management > Tracking", ["asset_inventory"]),
    "PE-21": ("Electromagnetic pulse protection", ["Implement EMP protection for critical systems", "Install EMP shielding on critical infrastructure", "Test EMP protection effectiveness", "Document EMP protection measures"], "Facilities > EMP Protection", ["config_snapshots"]),
    "PE-22": ("Component marking", ["Mark system components with security designations", "Implement asset tagging for all components", "Verify marking accuracy during inventory", "Update markings on component changes"], "Asset Management > Component Marking", ["asset_inventory"]),
    "PE-23": ("Facility location", ["Select facility location based on threat assessment", "Consider natural disaster, physical threat, and environmental factors", "Document location selection rationale", "Reassess location suitability on significant threat changes"], "Facilities > Site Selection", ["policy_document"]),
}
    ...
    if ctrl_id not in CONTROLS:
        CONTROLS[ctrl_id] = {
            ...
            "remediation_steps": [
                ...
                "Document implementation details" if "withdrawn" not in summ.lower() else "Verify replacement control is implemented",
                "Review and update annually" if "withdrawn" not in summ.lower() else "No additional action required",
                "Distribute to relevant stakeholders" if "withdrawn" not in summ.lower() else "Document withdrawal acknowledgment",
            ],
...
for ctrl_id, summ in PS_DEFS.items():
    if ctrl_id not in CONTROLS:
        steps = {
            "PS-1": ["Draft personnel security policy", "Define screening and access agreement requirements", "Establish personnel security review cycle", "Distribute policy to HR and management"],
            "PS-2": ["Assign risk designations to all positions", "Review risk designations when position duties change", "Document risk designation criteria", "Update designations at least every 3 years"],
            "PS-3": ["Conduct background screening before granting access", "Screen at level commensurate with position risk", "Re-screen personnel at defined intervals", "Document screening results"],
            "PS-4": ["Disable system access upon termination notification", "Retrieve all organizational assets and credentials", "Conduct exit interview covering security obligations", "Complete termination checklist within 24 hours"],
            "PS-5": ["Review and update access rights upon personnel transfer", "Initiate access modification within 24 hours of transfer", "Remove access no longer needed in new role", "Document transfer access changes"],
            "PS-6": ["Require signed access agreements before granting access", "Include acceptable use and security responsibilities", "Re-sign agreements annually or on significant policy changes", "Maintain signed agreement records"],
            "PS-7": ["Establish security requirements for external personnel", "Include security clauses in contracts", "Monitor external personnel compliance", "Terminate access for non-compliant external personnel"],
            "PS-8": ["Define sanctions for security policy violations", "Apply sanctions consistently per policy", "Document all sanctions and corrective actions", "Review sanctions process annually"],
            "PS-9": ["Include security responsibilities in position descriptions", "Define security role requirements per position", "Update descriptions when responsibilities change", "Review position descriptions annually"],
        }.get(ctrl_id, [f"Implement {summ.lower()}", "Document implementation", "Review annually", "Track compliance"])
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": steps,
            "console_path": "HR System > Personnel Security" if ctrl_id != "PS-1" else "Confluence > SEC Space > Personnel Security Policy",
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures"],
...
for ctrl_id, summ in RA_DEFS.items():
    if ctrl_id not in CONTROLS:
        steps_map = {
            "RA-1": ["Draft risk assessment policy", "Define risk assessment methodology (NIST RMF, FAIR)", "Establish assessment frequency and triggers", "Distribute policy to all personnel"],
            "RA-2": ["Categorize system using FIPS 199 criteria", "Document security categorization in SSP", "Review categorization on significant system changes", "Validate categorization with authorizing official"],
            "RA-3": ["Conduct annual risk assessment", "Identify threats, vulnerabilities, and impacts", "Calculate risk using approved methodology", "Document risk assessment results and recommendations"],
            "RA-4": ["This control has been withdrawn and incorporated into RA-3", "Verify RA-3 risk assessment controls are implemented"],
            "RA-5": ["Implement automated vulnerability scanning (Qualys, Tenable, Rapid7)", "Scan all systems at least monthly", "Remediate critical/high vulnerabilities within 30 days", "Track vulnerability metrics and report trends"],
            "RA-6": ["Conduct TSCM surveys for facilities handling sensitive information", "Schedule surveys based on threat assessment", "Document survey results and findings", "Address identified surveillance threats"],
            "RA-7": ["Define risk response strategies (accept, mitigate, transfer, avoid)", "Document risk response decisions", "Implement chosen risk response measures", "Monitor risk response effectiveness"],
            "RA-8": ["Conduct privacy impact assessments for systems processing PII", "Document privacy risks and mitigations", "Review PIAs on significant system changes", "Publish PIAs as required"],
            "RA-9": ["Conduct criticality analysis of system components", "Prioritize components by mission impact", "Document criticality designations", "Review criticality analysis on system changes"],
            "RA-10": ["Implement threat hunting program", "Conduct proactive threat hunts based on threat intelligence", "Document threat hunting activities and findings", "Improve detection capabilities based on hunting results"],
        }
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": steps_map.get(ctrl_id, [f"Implement {summ.lower()}", "Document implementation", "Review annually", "Track compliance"]),
            "console_path": "GRC Platform > Risk Assessment" if ctrl_id != "RA-5" else "Qualys > Scans | Tenable > Vulnerability Dashboard",
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures"],
            "evidence_types": ["risk_assessment", "policy_document"] if ctrl_id != "RA-5" else ["vulnerability_scans", "scan_reports"],
        }
    ...
    if ctrl_id not in CONTROLS:
        CONTROLS[ctrl_id] = {
            ...
            "remediation_steps": [
                ...
                "Document implementation requirements" if "withdrawn" not in summ.lower() else "Verify replacement control is implemented",
                "Review and update annually" if "withdrawn" not in summ.lower() else "No additional action required",
                "Verify compliance with acquisition requirements" if "withdrawn" not in summ.lower() else "Document withdrawal acknowledgment",
            ],
            "console_path": "GRC Platform > Acquisition" if ctrl_id != "SA-11" else "CI/CD > Security Testing | Snyk > Code | SonarQube",
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures"],
            "evidence_types": ["policy_document"] if ctrl_id != "SA-11" else ["code_scan_results", "test_reports"],
        }
    ...
    if ctrl_id not in CONTROLS:
        ...
        CONTROLS[ctrl_id] = {
            ...
            "remediation_steps": [
                ...
                "Verify replacement control is implemented" if is_withdrawn else "Configure security controls in network and system settings",
                "No additional action required" if is_withdrawn else "Verify effectiveness through testing",
                "Document withdrawal acknowledgment" if is_withdrawn else "Review and update configuration annually",
            ],
            "console_path": console,
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures"],
            "evidence_types": ["config_snapshots", "network_config"] if not is_withdrawn else ["policy_document"],
        }
...
for ctrl_id, summ in SI_BASE_DEFS.items():
    if ctrl_id not in CONTROLS:
        ...
        CONTROLS[ctrl_id] = {
            ...
            "remediation_steps": [
                "This control has been withdrawn" if is_withdrawn else f"Implement {summ.lower()}",
                "Verify replacement control is implemented" if is_withdrawn else "Configure monitoring and alerting",
                "No additional action required" if is_withdrawn else "Verify effectiveness through testing",
                "Document withdrawal acknowledgment" if is_withdrawn else "Review and update annually",
            ],
            "console_path": console,
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures"],
...
for ctrl_id, summ in SR_BASE_DEFS.items():
    if ctrl_id not in CONTROLS:
        CONTROLS[ctrl_id] = {
            ...
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures", "NIST SP 800-161: SCRM Practices"],
...
FAMILY_CONTEXT = {
    ...
    "AT": {"category": "administrative", "domain": "Awareness and Training", "console": "Training Platform"},
    "AU": {"category": "operational", "domain": "Audit and Accountability", "console": "SIEM > Audit"},
    "CA": {"category": "operational", "domain": "Assessment and Authorization", "console": "GRC Platform"},
    "CM": {"category": "operational", "domain": "Configuration Management", "console": "Configuration Management"},
    "CP": {"category": "operational", "domain": "Contingency Planning", "console": "Business Continuity"},
    "IA": {"category": "technical", "domain": "Identification and Authentication", "console": "Identity Provider"},
    "IR": {"category": "operational", "domain": "Incident Response", "console": "Incident Response"},
    ...
    "PE": {"category": "physical", "domain": "Physical and Environmental Protection", "console": "Facilities Management"},
    "PL": {"category": "administrative", "domain": "Planning", "console": "GRC Platform > Planning"},
    "PM": {"category": "administrative", "domain": "Program Management", "console": "GRC Platform > Program Management"},
    "PS": {"category": "administrative", "domain": "Personnel Security", "console": "HR System > Personnel Security"},
    "PT": {"category": "administrative", "domain": "PII Processing and Transparency", "console": "Privacy Portal"},
    "RA": {"category": "operational", "domain": "Risk Assessment", "console": "GRC Platform > Risk Assessment"},
    "SA": {"category": "operational", "domain": "System and Services Acquisition", "console": "GRC Platform > Acquisition"},
    "SC": {"category": "technical", "domain": "System and Communications Protection", "console": "Network > Security"},
    "SI": {"category": "technical", "domain": "System and Information Integrity", "console": "Security Center"},
    "SR": {"category": "operational", "domain": "Supply Chain Risk Management", "console": "Supply Chain Management"},
}

for ctrl_id in missing:
    family = ctrl_id.split("-")[0]
    ctx = FAMILY_CONTEXT.get(family, {"category": "operational", "domain": family, "console": "GRC Platform"})
...
# PE enhancements
PE_ENH_NAMES = {
    "PE-2(1)": "Access by position or role", "PE-2(2)": "Two forms of identification",
    "PE-2(3)": "Restrict unescorted access",
    "PE-3(1)": "System access", "PE-3(2)": "Facility and systems",
    "PE-3(3)": "Continuous guards", "PE-3(4)": "Lockable casings",
    "PE-3(5)": "Tamper protection", "PE-3(6)": "Facility penetration testing",
    "PE-3(7)": "Physical barriers", "PE-3(8)": "Access control vestibules",
    "PE-4(1)": "Electromagnetic emanation guards",
    "PE-5(1)": "Access to output by authorized individuals",
    "PE-5(2)": "Link to individual identity", "PE-5(3)": "Marking output devices",
    "PE-6(1)": "Intrusion alarms and surveillance", "PE-6(2)": "Automated intrusion recognition",
    "PE-6(3)": "Video surveillance", "PE-6(4)": "Monitoring physical access to systems",
    "PE-8(1)": "Automated records maintenance", "PE-8(2)": "Physical access records",
    "PE-8(3)": "Limit release of visitor access records",
    "PE-9(1)": "Redundant cabling", "PE-9(2)": "Automatic voltage controls",
    ...
    "PE-13(3)": "Automatic fire suppression", "PE-13(4)": "Inspections",
    "PE-14(1)": "Automatic controls", "PE-14(2)": "Monitoring with alarms and notifications",
...
MA_ENH = {
    ...
    "MA-3(1)": "Inspect maintenance tools", "MA-3(2)": "Inspect maintenance media",
    "MA-3(3)": "Prevent unauthorized removal of maintenance equipment",
    "MA-3(4)": "Restricted tool use", "MA-3(5)": "Execution with privilege",
    ...
    "MA-6(1)": "Preventive maintenance", "MA-6(2)": "Predictive maintenance",
...
for ctrl_id, summ in MA_ENH.items():
    if ctrl_id in CONTROLS and CONTROLS[ctrl_id]["summary"].startswith("Enhancement"):
        ...
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": [f"Implement {summ.lower()}", "Document maintenance procedures", "Verify through inspection", "Review annually"],
...
# MP enhancements
MP_ENH = {
    "MP-2(1)": "Automated restricted access", "MP-2(2)": "Cryptographic protection of media",
    "MP-3(1)": "Automated marking", "MP-3(2)": "Removable media marking",
    "MP-4(1)": "Cryptographic protection", "MP-4(2)": "Automated restricted access",
    "MP-5(1)": "Protection outside of controlled areas",
    "MP-5(2)": "Documentation of activities", "MP-5(3)": "Custodians",
    "MP-5(4)": "Cryptographic protection during transport",
    "MP-6(1)": "Review, approve, track, and verify media sanitization",
    "MP-6(2)": "Equipment testing", "MP-6(3)": "Nondestructive techniques",
    "MP-6(4)": "Controlled unclassified information", "MP-6(5)": "Classified information",
    "MP-6(6)": "Media destruction", "MP-6(7)": "Dual authorization",
    "MP-6(8)": "Remote purging or wiping",
    "MP-7(1)": "Prohibit use without owner", "MP-7(2)": "Prohibit use of sanitization-resistant media",
    "MP-8(1)": "Documentation of process", "MP-8(2)": "Equipment testing",
    "MP-8(3)": "Controlled unclassified information", "MP-8(4)": "Classified information",
}
...
for ctrl_id, summ in MP_ENH.items():
    if ctrl_id in CONTROLS and CONTROLS[ctrl_id]["summary"].startswith("Enhancement"):
        ...
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": [f"Implement {summ.lower()}", "Document media protection procedures", "Verify through inspection", "Review annually"],
...
# PL enhancements
PL_ENH = {
    "PL-2(1)": "Concept of operations (withdrawn)", "PL-2(2)": "Functional architecture (withdrawn)",
    ...
    "PL-8(1)": "Defense in depth", "PL-8(2)": "Supplier diversity",
}
...
for ctrl_id, summ in PL_ENH.items():
    if ctrl_id in CONTROLS and CONTROLS[ctrl_id]["summary"].startswith("Enhancement"):
        ...
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": [
                "This control has been withdrawn" if "withdrawn" in summ.lower() else f"Implement {summ.lower()}",
                "Verify replacement control is implemented" if "withdrawn" in summ.lower() else "Document planning requirements",
                "Review annually",
                "Track compliance",
            ],
...
# PM enhancements
PM_ENH = {
    "PM-5(1)": "Inventory of PII", "PM-7(1)": "Offloading",
    "PM-11(1)": "Ongoing authorization", "PM-14(1)": "Testing, training, and monitoring program",
    "PM-16(1)": "Automated threat awareness", "PM-25(1)": "Minimization of PII in testing",
...
for ctrl_id, summ in PM_ENH.items():
    if ctrl_id in CONTROLS and CONTROLS[ctrl_id]["summary"].startswith("Enhancement"):
        ...
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": [f"Implement {summ.lower()}", "Document program management details", "Review annually", "Track compliance"],
            "console_path": "GRC Platform > Program Management",
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures"],
            "evidence_types": ["policy_document"],
        }
...
# PS enhancements
PS_ENH = {
    "PS-3(1)": "Classified information screening", "PS-3(2)": "Formal indoctrination",
    "PS-3(3)": "Information requiring special protection", "PS-3(4)": "Citizenship requirements",
    "PS-4(1)": "Post-employment requirements", "PS-4(2)": "Automated actions on termination",
    "PS-5(1)": "Access restriction changes", "PS-5(2)": "Automated notification",
    ...
    "PS-9(1)": "Classified information positions", "PS-9(2)": "Position sensitivity levels",
}
...
for ctrl_id, summ in PS_ENH.items():
    if ctrl_id in CONTROLS and CONTROLS[ctrl_id]["summary"].startswith("Enhancement"):
        ...
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": [f"Implement {summ.lower()}", "Document personnel security procedures", "Review annually", "Track compliance"],
...
# PT enhancements
PT_ENH = {
    "PT-2(1)": "Data tagging for authority to process PII", "PT-2(2)": "Automation of authority tracking",
    "PT-3(1)": "Data tagging for PII processing purposes", "PT-3(2)": "Automation of purpose tracking",
    "PT-4(1)": "Tailored consent", "PT-4(2)": "Just-in-time consent", "PT-4(3)": "Revocation of consent",
    "PT-5(1)": "Just-in-time notice", "PT-5(2)": "Privacy act statements",
    "PT-6(1)": "Routine uses", "PT-6(2)": "Exemption rules",
    "PT-7(1)": "Social security numbers", "PT-7(2)": "First amendment information",
...
for ctrl_id, summ in PT_ENH.items():
    if ctrl_id in CONTROLS and CONTROLS[ctrl_id]["summary"].startswith("Enhancement"):
        ...
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": [f"Implement {summ.lower()}", "Document privacy requirements", "Review annually", "Track compliance"],
            "console_path": "Privacy Portal > PII Management",
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures"],
...
RA_ENH = {
    ...
    "RA-3(1)": "Supply chain risk assessment", "RA-3(2)": "Use of threat intelligence for risk assessment",
    "RA-3(3)": "Dynamic threat environment", "RA-3(4)": "Predictive cyber analytics",
    ...
    "RA-9(2)": "Supply chain criticality analysis", "RA-9(3)": "Identification of critical components",
...
for ctrl_id, summ in RA_ENH.items():
    if ctrl_id in CONTROLS and CONTROLS[ctrl_id]["summary"].startswith("Enhancement"):
        ...
        CONTROLS[ctrl_id] = {
            ...
            "remediation_steps": [
                "This control has been withdrawn" if "withdrawn" in summ.lower() else f"Implement {summ.lower()}",
                "Verify replacement control" if "withdrawn" in summ.lower() else "Configure risk assessment tools",
                "Review annually",
                "Track compliance",
            ],
            "console_path": "GRC Platform > Risk Assessment" if "RA-5" not in ctrl_id else "Qualys > Scans | Tenable > Dashboard",
            "recommended_reading": [f"NIST SP 800-53A Rev 5: {ctrl_id} Assessment Procedures"],
...
SR_ENH = {
    ...
    "SR-3(1)": "Diverse supply base", "SR-3(2)": "Limit harm from supply chain compromise",
    "SR-3(3)": "Sub-tier flow down",
    "SR-4(1)": "Identity of provenance", "SR-4(2)": "Track and trace",
    "SR-4(3)": "Validate as genuine and not altered", "SR-4(4)": "Supply chain integrity — pedigree",
    "SR-5(1)": "Adequate supply", "SR-5(2)": "Assessments prior to selection",
    ...
    "SR-11(1)": "Anti-counterfeit training", "SR-11(2)": "Configuration control for component service and repair",
...
for ctrl_id, summ in SR_ENH.items():
    if ctrl_id in CONTROLS and CONTROLS[ctrl_id]["summary"].startswith("Enhancement"):
        ...
        CONTROLS[ctrl_id] = {
            "summary": summ,
            "remediation_steps": [f"Implement {summ.lower()}", "Document supply chain procedures", "Review annually", "Track compliance"],
...
SA_ENH_SUMMS = {}
for i in range(1, 8):
    SA_ENH_SUMMS[f"SA-10({i})"] = {1: "Software/firmware integrity verification", 2: "Alternative configuration management processes", 3: "Hardware integrity verification", 4: "Trusted generation", 5: "Mapping integrity for version control", 6: "Trusted distribution", 7: "Security and privacy representatives"}[i]
for i in range(1, 10):
    SA_ENH_SUMMS[f"SA-11({i})"] = {1: "Static code analysis", 2: "Threat modeling and vulnerability analysis", 3: "Independent verification of assessment plans", 4: "Manual code reviews", 5: "Penetration testing", 6: "Attack surface reviews", 7: "Verify scope of testing", 8: "Dynamic code analysis", 9: "Interactive application security testing"}[i]
...
for i in range(1, 13):
    key = f"SA-4({i})"
    names = {1: "Functional properties of controls", 2: "Design and implementation for controls", 3: "Development methods, techniques, and practices", 4: "Assignment of components to systems (withdrawn)", 5: "System, component, and service configurations", 6: "Use of information assurance products", 7: "NIAP-approved protection profiles", 8: "Continuous monitoring plan for controls", 9: "Functions, ports, protocols, and services in use", 10: "Use of approved PIV products", 11: "System of records", 12: "Data ownership"}
    SA_ENH_SUMMS[key] = names.get(i, f"Acquisition requirement {i}")

for i in range(1, 6):
    SA_ENH_SUMMS[f"SA-5({i})"] = {1: "Functional properties of security controls documentation", 2: "Security-relevant external system interfaces", 3: "High-level design documentation", 4: "Low-level design documentation", 5: "Source code documentation"}[i]

# SA-9 enhancements
for i in range(1, 9):
    SA_ENH_SUMMS[f"SA-9({i})"] = {1: "Risk assessments and organizational approvals", 2: "Identification of functions, ports, protocols, and services", 3: "Establish and maintain trust relationship with providers", 4: "Consistent interests of consumers and providers", 5: "Processing, storage, and service location", 6: "Organization-controlled cryptographic keys", 7: "Organization-controlled integrity checking", 8: "Processing and storage location — US jurisdiction"}[i]

for i in range(1, 13):
    key = f"SA-15({i})"
    names = {1: "Quality metrics", 2: "Security and privacy tracking tools", 3: "Criticality analysis", 4: "Threat modeling and vulnerability analysis", 5: "Attack surface reduction", 6: "Continuous improvement", 7: "Automated vulnerability analysis", 8: "Reuse of threat and vulnerability information", 9: "Use of live data (withdrawn)", 10: "Incident response plan", 11: "Archive system or component", 12: "Minimize personally identifiable information"}
    if i in names:
        SA_ENH_SUMMS[key] = names[i]

for i in range(1, 10):
    key = f"SA-17({i})"
    names = {1: "Formal policy model", 2: "Security-relevant components", 3: "Formal correspondence", 4: "Informal correspondence", 5: "Conceptually simple design", 6: "Structure for testing", 7: "Structure for least privilege", 8: "Orchestration", 9: "Design diversity"}
    if i in names:
        SA_ENH_SUMMS[key] = names[i]
...
for i in range(1, 16):
    SA_ENH_SUMMS[f"SA-12({i})"] = f"Supply chain protection enhancement {i} (withdrawn - see SR family)"

for ctrl_id, summ in SA_ENH_SUMMS.items():
    if ctrl_id in CONTROLS:
        if CONTROLS[ctrl_id]["summary"].startswith("Enhancement") or CONTROLS[ctrl_id]["summary"].startswith("Supply chain"):
            CONTROLS[ctrl_id]["summary"] = summ
...
for i in range(1, 30):
    names = {1: "Physically separated subnetworks", 2: "Public access", 3: "Access points",
             4: "External telecommunications services", 5: "Deny by default/allow by exception",
             6: "Response to recognized failures (withdrawn)", 7: "Split tunneling for remote devices",
             8: "Route traffic to authenticated proxy servers",
             9: "Restrict threatening outgoing traffic", 10: "Prevent exfiltration",
             11: "Restrict incoming communications traffic", 12: "Host-based protection",
             13: "Isolation of security tools", 14: "Protection against unauthorized physical connections",
             15: "Networked privileged accesses", 16: "Prevent discovery of system components",
             17: "Automated enforcement of protocol formats", 18: "Fail secure",
             19: "Block communication from non-organizationally configured hosts",
             20: "Dynamic isolation and segregation", 21: "Isolation of system components",
             ...
             24: "Personally identifiable information", 25: "Connections in public environments",
             26: "Classified national security system connections",
             27: "Unclassified non-national security system connections",
             28: "Connections to public networks", 29: "Separate subnets to isolate functions"}
    if i in names:
...
# SC-12 enhancements
for i in range(1, 7):
    names = {1: "Availability of key management", 2: "Symmetric keys", 3: "Asymmetric keys", 4: "PKI certificates (withdrawn)", 5: "PKI certificates (withdrawn)", 6: "Physical control of keys"}
    SC_ENH_SUMMS[f"SC-12({i})"] = names.get(i, f"Key management enhancement {i}")

# SC-13 enhancements
for i in range(1, 5):
    names = {1: "FIPS-validated cryptography", 2: "NSA-approved cryptography", 3: "Individuals without formal access", 4: "Digital signatures"}
    SC_ENH_SUMMS[f"SC-13({i})"] = names.get(i, f"Cryptographic protection enhancement {i}")

# Additional SC enhancements
SC_MISC = {
    "SC-2(1)": "Interfaces for non-privileged users", "SC-2(2)": "Disassociability",
    "SC-3(1)": "Hardware separation", "SC-3(2)": "Access and flow control",
    "SC-3(3)": "Minimize nonsecurity functionality", "SC-3(4)": "Module coupling and cohesiveness",
    "SC-3(5)": "Layered structures",
    "SC-4(1)": "Security levels", "SC-4(2)": "Periods processing",
    "SC-5(1)": "Restrict ability to attack other systems", "SC-5(2)": "Capacity, bandwidth, and redundancy",
    "SC-5(3)": "Detection and monitoring",
    "SC-10(1)": "RPC timeout",
    "SC-16(1)": "Integrity verification", "SC-16(2)": "Anti-spoofing mechanisms",
    "SC-16(3)": "Cryptographic mechanisms for attributes",
    "SC-17(1)": "Certificate status mechanism",
    "SC-18(1)": "Identify unacceptable mobile code", "SC-18(2)": "Acquisition, development, and use",
    "SC-18(3)": "Prevent downloading and execution", "SC-18(4)": "Prevent automatic execution",
    ...
    "SC-36(1)": "Polling techniques", "SC-36(2)": "Synchronization",
    "SC-39(1)": "Hardware separation", "SC-39(2)": "Separate execution domain per thread",
...
# SI-2 enhancements
for i in range(1, 7):
    names = {1: "Central management of flaw remediation", 2: "Automated flaw remediation status", 3: "Time to remediate flaws and benchmarks", 4: "Automated patch management tools", 5: "Automatic software and firmware updates", 6: "Removal of previous versions of software and firmware"}
    SI_ENH_SUMMS[f"SI-2({i})"] = names.get(i, f"Flaw remediation enhancement {i}")

# SI-3 enhancements
for i in range(1, 11):
    names = {1: "Central management of malicious code protection", 2: "Automatic updates", 3: "Non-privileged users (withdrawn)", 4: "Updates only by privileged users", 5: "Portable storage devices (withdrawn)", 6: "Testing and verification", 7: "Nonsignature-based detection", 8: "Detect unauthorized commands", 9: "Authenticate remote commands", 10: "Malicious code analysis"}
    SI_ENH_SUMMS[f"SI-3({i})"] = names.get(i, f"Malicious code protection enhancement {i}")

# SI-4 enhancements
for i in range(1, 26):
    names = {1: "System-wide intrusion detection", 2: "Automated analysis and integration", 3: "Automated tool and mechanism integration",
             4: "Inbound and outbound communications traffic", 5: "System-generated alerts",
             6: "Restrict non-privileged users (withdrawn)", 7: "Automated response to suspicious events",
             8: "Protection of monitoring information (withdrawn)", 9: "Testing of monitoring tools and mechanisms",
             10: "Visibility of encrypted communications", 11: "Analyze communications traffic anomalies",
             12: "Automated organization-generated alerts", 13: "Analyze traffic and event patterns",
             14: "Wireless intrusion detection", 15: "Wireless to wired communications",
             16: "Correlate monitoring information", 17: "Integrated situational awareness",
             18: "Analyze traffic and covert exfiltration", 19: "Risk for individuals",
             20: "Privileged users", 21: "Probationary periods",
             22: "Unauthorized network services", 23: "Host-based devices",
             24: "Indicators of compromise", 25: "Optimize network traffic analysis"}
    if i in names:
...
# SI-6 enhancements
for i in range(1, 4):
    names = {1: "Notification of failed security tests (withdrawn)", 2: "Automation support for distributed testing", 3: "Report verification results"}
    SI_ENH_SUMMS[f"SI-6({i})"] = names.get(i, f"Verification enhancement {i}")

# SI-7 enhancements
for i in range(1, 18):
    names = {1: "Integrity checks", 2: "Automated notifications of integrity violations", 3: "Centrally managed integrity tools",
             4: "Tamper-evident packaging (withdrawn)", 5: "Automated response to integrity violations",
             6: "Cryptographic protection", 7: "Integration of detection and response",
             8: "Auditing capability for significant events", 9: "Verify boot process",
             10: "Protection of boot firmware", 11: "Confined environments with limited privileges",
             12: "Integrity verification", 13: "Code execution in protected environments",
             14: "Binary or machine executable code", 15: "Code authentication",
             16: "Time limit on process execution without supervision", 17: "Runtime application self-protection"}
    if i in names:
        SI_ENH_SUMMS[f"SI-7({i})"] = names[i]

# SI-10 enhancements
for i in range(1, 7):
    names = {1: "Manual override capability", 2: "Review and resolve errors", 3: "Predictive and reactive validation",
             4: "Timing interactions", 5: "Restrict inputs to trusted sources", 6: "Injection prevention"}
    SI_ENH_SUMMS[f"SI-10({i})"] = names.get(i, f"Input validation enhancement {i}")

# SI-12 enhancements
for i in range(1, 4):
    names = {1: "Limit personally identifiable information elements", 2: "Minimize PII in testing", 3: "Information disposal"}
    SI_ENH_SUMMS[f"SI-12({i})"] = names.get(i, f"Information management enhancement {i}")

# SI-16 enhancements
for i in range(1, 4):
    SI_ENH_SUMMS[f"SI-16({i})"] = {1: "Stack protection", 2: "Heap protection", 3: "Code segment protection"}.get(i, f"Memory protection enhancement {i}")

# SI-18 enhancements
for i in range(1, 6):
    names = {1: "Automation support", 2: "Data tags", 3: "Collection", 4: "Individual requests", 5: "Notice of correction or deletion"}
    SI_ENH_SUMMS[f"SI-18({i})"] = names.get(i, f"PII quality enhancement {i}")

# SI-19 enhancements
for i in range(1, 9):
    names = {1: "Collection", 2: "Archiving", 3: "Release", 4: "Removal, masking, or alteration of direct identifiers",
             5: "Statistical disclosure control", 6: "Differential privacy", 7: "Validated algorithms and software",
...
for ctrl_id in sorted_controls:
    ...
    if ctrl is None:
        ...
        ctrl = {
            ...
            "remediation_steps": [f"Implement {ctrl_id} requirements", "Document implementation", "Review annually", "Track compliance"],
...
with open(output_path, "w") as f:
    yaml.dump(output, f, Dumper=NullDumper, default_flow_style=False, sort_keys=False, width=120, allow_unicode=True)
